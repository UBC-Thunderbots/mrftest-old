...
BoolParam send_tune_updates(
    u8"Whether or not to broadcast variable tune updates", u8"MRF/Robot/",
    false);
...
unsigned int chicker_power_to_pulse_width(double power, bool chip)
{
    ...
    else
    {
        ...
        width =
            static_cast<unsigned>(835 * power * power + 469.2 * power + 1118.5);
    }
...
void MRFRobot::move_move(Point dest, double end_speed)
{
    assert(!direct_control);
    ...
    params[2] = 0.0;
    params[3] = end_speed * 1000.0;
    extra     = 0;
    dirty_drive();
}

void MRFRobot::move_move(Point dest, Angle orientation, double end_speed)
{
    assert(!direct_control);
    primitive = Drive::Primitive::MOVE;
    params[0] = dest.x * 1000.0;
...
void MRFRobot::move_shoot(Point dest, double power, bool chip)
{
    ...
    params[1] = dest.y * 1000.0;
    params[2] = 0.0;
    uint16_t chick_power =
        static_cast<uint16_t>(chicker_power_to_pulse_width(power, chip));
    params[3] = chick_power;
    extra     = chip;
    dirty_drive();
}

void MRFRobot::move_shoot(
...
MRFRobot::MRFRobot(MRFDongle &dongle, unsigned int index)
    : Drive::Robot(index, 0.3, 10.0, 2.0, MAX_DRIBBLER_RPM),
      dongle_(dongle),
      low_capacitor_message(
          Glib::ustring::compose(u8"Bot %1 low caps (fuse blown?)", index),
          Annunciator::Message::TriggerMode::LEVEL,
          Annunciator::Message::Severity::HIGH),
      fw_build_id_mismatch_message(
          u8"", Annunciator::Message::TriggerMode::LEVEL,
          Annunciator::Message::Severity::LOW),
      fpga_build_id_mismatch_message(
          u8"", Annunciator::Message::TriggerMode::LEVEL,
          Annunciator::Message::Severity::LOW),
      build_id_fetch_error_message(
          Glib::ustring::compose(u8"Bot %1 failed to read build IDs", index),
          Annunciator::Message::TriggerMode::LEVEL,
          Annunciator::Message::Severity::LOW),
      ...
      request_build_ids_counter(REQUEST_BUILD_IDS_COUNT)
{
    ...
    {
        error_lt_messages[i].reset(new Annunciator::Message(
            Glib::ustring::compose(
                u8"Bot %1 %2", index, MRF::ERROR_LT_MESSAGES[i]),
            Annunciator::Message::TriggerMode::LEVEL,
            Annunciator::Message::Severity::HIGH));
    }
    for (unsigned int i = 0; i < MRF::ERROR_ET_COUNT; ++i)
    {
        error_et_messages[i].reset(new Annunciator::Message(
            Glib::ustring::compose(
                u8"Bot %1 %2", index, MRF::ERROR_ET_MESSAGES[i]),
            Annunciator::Message::TriggerMode::EDGE,
            Annunciator::Message::Severity::HIGH));
    }

    static_assert(
        sizeof(SD_MESSAGES) / sizeof(*SD_MESSAGES) ==
            MRFRobot::SD_MESSAGE_COUNT,
        "Wrong number of SD message initializers");
    for (std::size_t i = 0; i < sizeof(SD_MESSAGES) / sizeof(*SD_MESSAGES); ++i)
    {
        if (SD_MESSAGES[i])
        {
            sd_messages[i].reset(new Annunciator::Message(
                Glib::ustring::compose(SD_MESSAGES[i], index),
                Annunciator::Message::TriggerMode::LEVEL,
                Annunciator::Message::Severity::HIGH));
        }
    }
    ...
    static_assert(
        sizeof(LOGGER_MESSAGES) / sizeof(*LOGGER_MESSAGES) ==
            MRFRobot::LOGGER_MESSAGE_COUNT,
        "Wrong number of SD message initializers");
    for (std::size_t i = 0;
         i < sizeof(LOGGER_MESSAGES) / sizeof(*LOGGER_MESSAGES); ++i)
    {
        if (LOGGER_MESSAGES[i])
        {
            logger_messages[i].reset(new Annunciator::Message(
                Glib::ustring::compose(LOGGER_MESSAGES[i], index),
                Annunciator::Message::TriggerMode::LEVEL,
                Annunciator::Message::Severity::HIGH));
        }
    }

    direct_control.signal_changed().connect(
        sigc::mem_fun(this, &MRFRobot::handle_direct_control_changed));
}
...
void MRFRobot::check_build_id_mismatch()
{
    static const struct
    {
        ...
        {u8"FW", &Drive::Robot::fw_build_id,
         &MRFRobot::fw_build_id_mismatch_message},
        {u8"FPGA", &Drive::Robot::fpga_build_id,
         &MRFRobot::fpga_build_id_mismatch_message},
    };
    ...
    for (const auto &field : FIELDS)
    {
        ...
        std::array<
            std::pair<unsigned int, uint32_t>,
            sizeof(dongle_.robots) / sizeof(*dongle_.robots)>
            ids;
        ...
        // Activate the warning message on all robots that do not match the
        // majority build ID.
        for (const std::unique_ptr<MRFRobot> &bot : dongle_.robots)
        {
            if (bot->build_ids_valid &&
                bot.get()->*field.field != ids[next - 1].second)
            {
                (bot.get()->*field.message)
                    .set_text(Glib::ustring::compose(
                        u8"Bot %1 %2 build ID 0x%3 mismatches majority 0x%4",
                        bot->index, field.name,
                        tohex(bot.get()->*field.field, 8),
                        tohex(ids[next - 1].second)));
