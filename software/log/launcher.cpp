...
void get_filenames(std::vector<std::string> &files)
{
    ...
    files.clear();
    std::copy_if(
        dir.begin(), dir.end(), std::back_inserter(files),
        [](const std::string &s) {
            return !s.empty() && s[0] != '.' &&
                   LogLoader::is_current_version(filename_to_pathname(s));
        });
...
LogLauncher::LogLauncher()
    : log_list(1, false, Gtk::SELECTION_MULTIPLE),
      analyzer_button(u8"Analyzer"),
      player_button(u8"Player"),
      rename_button(u8"Rename"),
      delete_button(u8"Delete"),
      export_button(u8"Export"),
      import_button(u8"Import"),
      exit_pending(false)
{
    ...
    log_list.get_selection()->signal_changed().connect(
        sigc::mem_fun(this, &LogLauncher::on_log_list_selection_changed));
    on_log_list_selection_changed();
    analyzer_button.signal_clicked().connect(
        sigc::mem_fun(this, &LogLauncher::on_analyzer_clicked));
    player_button.signal_clicked().connect(
        sigc::mem_fun(this, &LogLauncher::on_player_clicked));
    rename_button.signal_clicked().connect(
        sigc::mem_fun(this, &LogLauncher::on_rename_clicked));
    delete_button.signal_clicked().connect(
        sigc::mem_fun(this, &LogLauncher::on_delete_clicked));
    export_button.signal_clicked().connect(
        sigc::mem_fun(this, &LogLauncher::on_export_clicked));
    import_button.signal_clicked().connect(
        sigc::mem_fun(this, &LogLauncher::on_import_clicked));

    show_all();

    compress_lock_fd = FileDescriptor::create_open(
        filename_to_pathname(".lock").c_str(), O_RDWR | O_CREAT, 0666);
    ...
    else
    {
        // Delete old dotfiles in the logs directory.
        {
            ...
            std::copy_if(
                dir.begin(), dir.end(), std::back_inserter(to_delete),
                [](const std::string &fn) {
                    return !fn.empty() && fn[0] == '.' && fn != ".lock";
                });
            std::for_each(
                to_delete.begin(), to_delete.end(), [](const std::string &fn) {
                    std::remove(filename_to_pathname(fn).c_str());
                });
        }
        ...
        if (!files_to_compress.empty())
        {
            ...
            compress_dispatcher.connect(
                sigc::mem_fun(this, &LogLauncher::start_compressing));
            start_compressing();
        }
        else
        {
...
void LogLauncher::populate()
{
    ...
    {
        log_list.append(
            Glib::filename_display_basename(filename_to_pathname(i)));
    }
}
...
void LogLauncher::start_compressing()
{
    // Join and erase any threads that are finished.
    {
        ...
        {
            for (auto j    = compress_threads.begin(),
                      jend = compress_threads.end();
                 j != jend; ++j)
            {
                if (j->get_id() == i)
                {
    ...
    // Update progress bar to show # of files completely finished (excludes
    // those left to compress *and* those currently compressing).
    std::size_t files_done =
        static_cast<std::size_t>(std::distance(
            static_cast<const std::vector<std::string> &>(files_to_compress)
                .begin(),
            next_file_to_compress)) -
        compress_threads.size();
    compress_progress_bar.set_text(Glib::ustring::compose(
        u8"Compressed %1 / %2", files_done, files_to_compress.size()));
    compress_progress_bar.set_fraction(
        static_cast<double>(files_done) /
        static_cast<double>(files_to_compress.size()));

    // Fork more threads as long as we're supposed to.
    while (!exit_pending && next_file_to_compress != files_to_compress.end() &&
           (compress_threads.empty() ||
            compress_threads.size() < std::thread::hardware_concurrency()))
    {
        compress_threads.push_back(std::thread(std::bind(
            std::mem_fun(&LogLauncher::compress_thread_proc), this,
            *next_file_to_compress)));
        ++next_file_to_compress;
    }
...
void LogLauncher::compress_thread_proc(const std::string &filename)
{
    ...
    const std::string &temp_filename =
        Glib::filename_from_utf8(Glib::ustring::compose(
            u8".loglauncher.tmp.%1", std::this_thread::get_id()));
    const std::string &temp_pathname = filename_to_pathname(temp_filename);
    FileDescriptor dst_fd            = FileDescriptor::create_open(
        temp_pathname.c_str(), O_RDWR | O_CREAT | O_EXCL, 0666);

    {
        ...
        if (ftruncate(
                dst_fd.fd(), static_cast<off_t>(
                                 src_mapping.size() +
                                 (src_mapping.size() + 99) / 100 + 600)) < 0)
        {
        ...
        // Memory map the destination file and do a buffer-to-buffer compress.
        {
            ...
            if (BZ2_bzBuffToBuffCompress(
                    static_cast<char *>(dst_mapping.data()), &dlen,
                    const_cast<char *>(
                        static_cast<const char *>(src_mapping.data())),
                    static_cast<unsigned int>(src_mapping.size()), 9, 0,
                    0) != BZ_OK)
            {
            ...
            // Now shorten the destination file to the amount of space actually
            // used.
    ...
    if (std::rename(
            temp_pathname.c_str(), filename_to_pathname(filename).c_str()) < 0)
    {
...
void LogLauncher::on_analyzer_clicked()
{
    ...
    for (int i : selected)
    {
        try
        {
            new LogAnalyzer(
                *this,
                filename_to_pathname(files[static_cast<std::size_t>(i)]));
        }
        catch (const std::runtime_error &exp)
        {
            Gtk::MessageDialog md(
                *this, exp.what(), false, Gtk::MESSAGE_ERROR, Gtk::BUTTONS_OK,
                true);
            md.run();
        }
    }
...
void LogLauncher::on_rename_clicked()
{
    ...
    new_name_entry.set_text(log_list.get_text(
        static_cast<unsigned int>(log_list.get_selected()[0])));
    ...
    if (resp == Gtk::RESPONSE_OK)
    {
        const std::string &old_name =
            files[static_cast<std::size_t>(log_list.get_selected()[0])];
        const std::string &new_name =
            Glib::filename_from_utf8(new_name_entry.get_text());
        if (new_name != old_name)
        {
            ...
            {
                Gtk::MessageDialog md(
                    *this, exp.what(), false, Gtk::MESSAGE_ERROR,
                    Gtk::BUTTONS_OK, true);
                md.run();
            }
            populate();
    ...
    Gtk::MessageDialog md(
        *this,
        Glib::ustring::compose(
            u8"Are you sure you wish to delete %1 %2 log%3?",
            num == 1 ? u8"this" : u8"these", num, num == 1 ? u8"" : u8"s"),
...
void LogLauncher::on_export_clicked()
{
    ...
    Gtk::FileChooserDialog fcd(
        *this, u8"Export Log", Gtk::FILE_CHOOSER_ACTION_SAVE);
    ...
    {
        Gio::File::create_for_path(filename_to_pathname(original_name))
            ->copy(fcd.get_file(), Gio::FILE_COPY_OVERWRITE);
    }
}
...
{
    Gtk::FileChooserDialog fcd(
        *this, u8"Import Log", Gtk::FILE_CHOOSER_ACTION_OPEN);
    ...
    if (fcd.run() == Gtk::RESPONSE_OK)
    {
        for (Glib::RefPtr<Gio::File> source : fcd.get_files())
        {
            Glib::RefPtr<Gio::File> dest = Gio::File::create_for_path(
                filename_to_pathname(source->get_basename()));
            try
            {
            ...
            catch (const Gio::Error &err)
            {
                if (err.code() == Gio::Error::EXISTS)
                {
                    Gtk::MessageDialog md(
                        *this,
                        Glib::ustring::compose(
                            u8"A log file named “%1” already exists. Replace "
                            u8"it?",
                            dest->query_info(
