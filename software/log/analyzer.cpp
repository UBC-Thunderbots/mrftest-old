...
{
/**
 * \brief A mapping from transmitted byte to descriptive text of referee box
 * command.
 */
const struct
{
...
const struct
{
    ...
    {AI::Flags::MoveFlags::AVOID_FRIENDLY_DEFENSE,
     u8"Avoid friendly defense area"},
    {AI::Flags::MoveFlags::AVOID_ENEMY_DEFENSE, u8"Avoid enemy defense area"},
    {AI::Flags::MoveFlags::STAY_OWN_HALF, u8"Stay on own half"},
    {AI::Flags::MoveFlags::PENALTY_KICK_FRIENDLY,
     u8"Non-kicker in friendly penalty kick"},
    {AI::Flags::MoveFlags::PENALTY_KICK_ENEMY,
     u8"Non-kicker in enemy penalty kick"},
    {AI::Flags::MoveFlags::CAREFUL, u8"Careful"},
    {AI::Flags::MoveFlags::AVOID_BALL_MEDIUM, u8"Avoid ball (medium)"},
};

/**
 * \brief Converts a Log::MonotonicTimeSpec to a printable form suitable for
 * human consumption.
 *
...
Glib::ustring timespec_to_string(const Log::MonotonicTimeSpec &ts)
{
    if (ts.seconds() >= 0)
    {
        return Glib::ustring::compose(
            u8"%1.%2", ts.seconds(),
            todecu(static_cast<unsigned long>(ts.nanoseconds()), 9));
    }
    ...
    else
    {
        return Glib::ustring::compose(
            u8"-%1.%2", -(ts.seconds() + 1),
            todecu(
                1000000000UL - static_cast<unsigned long>(ts.nanoseconds()),
                9));
    }
}
...
Glib::ustring timespec_to_string_machine(const Log::MonotonicTimeSpec &ts)
{
    ...
    {
        return Glib::ustring::compose(
            u8"%1.%2", todecu(static_cast<uintmax_t>(ts.seconds()), 0),
            todecu(static_cast<unsigned long>(ts.nanoseconds()), 9));
    }
    else if (ts.nanoseconds() == 0)
    {
        return Glib::ustring::compose(
            u8"-%1.000000000",
            todecu(static_cast<uintmax_t>(-ts.seconds()), 0));
    }
    else
    {
        return Glib::ustring::compose(
            u8"-%1.%2", todecu(static_cast<uintmax_t>(-(ts.seconds() + 1)), 0),
            todecu(
                1000000000UL - static_cast<unsigned long>(ts.nanoseconds()),
                9));
    }
...
void tsv_writer_ball(
    std::ostream &os, const std::vector<Log::Record> &records, Gtk::Window &)
{
        ...
        else if (record.has_vision())
        {
            ...
            if (wrapper.has_detection())
            {
                ...
                for (int j = 0; j < frame.balls_size(); ++j)
                {
                    ...
                    os << "Vision";
                    os << '\t'
                       << timespec_to_string_machine(vision.timestamp());
                    os << '\t' << (ball.x() / 1000.0);
                    os << '\t' << (ball.y() / 1000.0);
                    os << '\t' << ball.confidence();
                    os << "\tX";
...
void tsv_writer_friendly_player_lps(
    std::ostream &os, const std::vector<Log::Record> &records,
    Gtk::Window &parent)
{
        ...
        else if (record.has_tick())
        {
            ...
            for (int j = 0; j < tick.friendly_robots_size(); ++j)
            {
                ...
                if (wrapper.has_detection()) {
                        const SSL_DetectionFrame &frame = wrapper.detection();
                        int count;
                        const SSL_DetectionRobot &
        (SSL_DetectionFrame::*fp)(int) const;
...
void tsv_writer_friendly_player(
    std::ostream &os, const std::vector<Log::Record> &records,
    Gtk::Window &parent)
{
    unsigned int pattern;
...
void tsv_writer_friendly_players(
    std::ostream &os, const std::vector<Log::Record> &records,
    Gtk::Window &parent)
{
    {
        ...
        Gtk::HBox hb;
        Gtk::Label label(
            u8"Generating logs for friendly players. Please have patience...");
        hb.pack_start(label, Gtk::PACK_SHRINK);
        hb.show_all();
        dlg.get_vbox()->pack_start(hb, Gtk::PACK_SHRINK);
        dlg.run();
...
{
    {
        ...
        Gtk::HBox hb;
        Gtk::Label label(
            u8"Generating logs for enemy players. Please have patience...");
        hb.pack_start(label, Gtk::PACK_SHRINK);
        hb.show_all();
        dlg.get_vbox()->pack_start(hb, Gtk::PACK_SHRINK);
        dlg.run();
    ...
    std::ostream &os, const std::vector<Log::Record> &records, Gtk::Window &)
{
    for (const Log::Record &record : records)
    {
        ...
        {
            os << "Tick\t"
               << timespec_to_string_machine(record.tick().start_time())
               << '\n';
        }
        else if (record.has_vision())
        {
            os << "Vision\t"
               << timespec_to_string_machine(record.vision().timestamp())
               << '\n';
        }
        else if (record.has_refbox())
        {
            os << "Refbox\t"
               << timespec_to_string_machine(record.refbox().timestamp())
               << '\n';
        }
    }
}

void tsv_writer_compute_times(
    std::ostream &os, const std::vector<Log::Record> &records, Gtk::Window &)
{
    for (const Log::Record &record : records)
    {
    ...
    std::ostream &os, const std::vector<Log::Record> &records, Gtk::Window &)
{
    ...
    os << "Label\ttimestamp\tball_x\tball_y\tconfidence\tball_vx\tball_vy"
       << "\tf1_x\tf1_y\tf1_vx\tf1_vy\tf2_x\tf2_y\tf2_vx\tf2_vy\tf3_x\tf3_"
          "y\tf3_vx\tf3_vy\tf4_x\tf4_y\tf4_vx\tf4_vy\tf5_x\tf5_y\tf5_vx\tf5_"
          "vy\tf6_x\tf6_y\tf6_vx\tf6_vy"
       << "\te1_x\te1_y\te1_vx\te1_vy\te2_x\te2_y\te2_vx\te2_vy\te3_x\te3_"
          "y\te3_vx\te3_vy\te4_x\te4_y\te4_vx\te4_vy\te5_x\te5_y\te5_vx\te5_"
          "vy\te6_x\te6_y\te6_vx\te6_vy"
       << "\n";
    ...
    for (const Log::Record &record : records)
    {
        ...
        else if (record.has_tick())
        {
            ...
            for (int j = 0; j < robots_team_size; ++j)
            {
                ...
                {
                    const Log::Tick::FriendlyRobot &bot =
                        tick.friendly_robots(j);
                    os << '\t' << (bot.position().x() / 1000000.0);
                    os << '\t' << (bot.position().y() / 1000000.0);
                    os << '\t' << (bot.velocity().x() / 1000000.0);
                    os << '\t' << (bot.velocity().y() / 1000000.0);
...
{
    std::function<void(
        std::ostream &, const std::vector<Log::Record> &, Gtk::Window &)>
        fn;
    const char *description;
} TSV_WRITERS[] = {
    {&tsv_writer_ball, u8"Ball position/velocity (predicted and vision)"},
    {&tsv_writer_friendly_player,
     u8"Friendly player position/velocity (predicted and vision)"},
    {&tsv_writer_friendly_players,
     u8"Friendly players position/velocity (predicted and vision)"},
    {&tsv_writer_friendly_player_lps, u8"Friendly player position, lps value"},
    {&tsv_writer_enemy_player,
     u8"Enemy player position/velocity (predicted and vision)"},
    {&tsv_writer_enemy_players,
     u8"Enemy players position/velocity (predicted and vision)"},
    {&tsv_writer_stamps, u8"Tick/vision/refbox timestamps"},
    {&tsv_writer_compute_times, u8"Tick computation times"},
    {&tsv_writer_all,
     u8"Ball pos/vel (predicted & vision), all player pos/vel"},
};
...
/**
 * \brief A column record containing two columns, a key and a value, suitable
 * for displaying decoded packet structures.
 */
class PacketDecodedTreeColumns final : public Gtk::TreeModelColumnRecord
{
   public:
     ...
     * \return the new row.
     */
    Gtk::TreeRow append_kv(
        Glib::RefPtr<Gtk::TreeStore> store, const Glib::ustring &k,
        const Glib::ustring &v = u8"") const;
     ...
     */
    Gtk::TreeRow append_kv(
        Glib::RefPtr<Gtk::TreeStore> store, const Gtk::TreeRow &parent,
        const Glib::ustring &k, const Glib::ustring &v = u8"") const;
};
...
Gtk::TreeRow PacketDecodedTreeColumns::append_kv(
    Glib::RefPtr<Gtk::TreeStore> store, const Glib::ustring &k,
    const Glib::ustring &v) const
{
...
Gtk::TreeRow PacketDecodedTreeColumns::append_kv(
    Glib::RefPtr<Gtk::TreeStore> store, const Gtk::TreeRow &parent,
    const Glib::ustring &k, const Glib::ustring &v) const
{
...
class RecordsALM final : public AbstractListModel, public Glib::Object
{
   ...
   private:
    ...
    explicit RecordsALM(const std::vector<Log::Record> &records)
        : Glib::ObjectBase(typeid(RecordsALM)), records(records)
    {
...
Glib::ustring build_decoded_tree_format_string(
    const std::string &data, google::protobuf::FieldDescriptor::Type type)
{
    switch (type)
    {
        ...
        case google::protobuf::FieldDescriptor::TYPE_BYTES:
        {
            ...
            for (char ch : data)
            {
                ...
                ustr.append(
                    tohex(static_cast<unsigned>(static_cast<uint8_t>(ch)), 2));
            }
            return ustr;
        }
...
void build_decoded_tree_nontoplevel(
    const google::protobuf::Message &message, Glib::RefPtr<Gtk::TreeStore> ts,
    PacketDecodedTreeColumns &cols, Gtk::TreeRow parent);

void build_decoded_tree_field_singular(
    const google::protobuf::Message &message,
    const google::protobuf::FieldDescriptor &fd,
    Glib::RefPtr<Gtk::TreeStore> ts, PacketDecodedTreeColumns &cols,
    Gtk::TreeRow &row)
{
    const google::protobuf::Reflection &refl = *message.GetReflection();

    if (dynamic_cast<const Log::UNIXTimeSpec *>(&message) &&
        fd.name() == "seconds")
    {
...
#pragma GCC diagnostic ignored                                                 \
    "-Wuseless-cast"  // The cast to time_t is useless on 64-bit platforms but
                      // not on 32-bit platforms.
...
#pragma GCC diagnostic pop
        ...
        dynamic_cast<const Log::Tick::FriendlyRobot *>(&message) &&
        fd.name() == "movement_flags")
    {
        ...
        for (const auto &i : MOVE_FLAG_MAPPING)
        {
            if (flags & static_cast<uint64_t>(i.mask))
            {
                ...
                cols.append_kv(
                    ts, row, i.description,
                    tohex(static_cast<uint64_t>(i.mask), 16));
            }
        }
        if (flags)
        {
        ...
        dynamic_cast<const Log::Refbox *>(&message) &&
        fd.name() == "legacy_data")
    {
        ...
        if (!found)
        {
            cols.append_kv(
                ts, row, u8"Command",
                Glib::ustring::compose(u8"Unknown (%1)", data[0]));
        }
        cols.append_kv(
            ts, row, u8"Counter",
            Glib::ustring::format(
        ...
        cols.append_kv(
            ts, row, u8"Goals Blue",
            Glib::ustring::format(
                static_cast<unsigned int>(static_cast<uint8_t>(data[2]))));
        cols.append_kv(
            ts, row, u8"Goals Yellow",
            Glib::ustring::format(
                static_cast<unsigned int>(static_cast<uint8_t>(data[3]))));
        cols.append_kv(
            ts, row, u8"Seconds Left",
            Glib::ustring::format(decode_u16_be(&data.data()[4])));
    }
    ...
    switch (fd.cpp_type())
    {
        case google::protobuf::FieldDescriptor::CPPTYPE_INT32:
            row[cols.value] =
                Glib::ustring::format(refl.GetInt32(message, &fd));
            return;

        case google::protobuf::FieldDescriptor::CPPTYPE_INT64:
            row[cols.value] =
                Glib::ustring::format(refl.GetInt64(message, &fd));
            return;

        case google::protobuf::FieldDescriptor::CPPTYPE_UINT32:
            row[cols.value] =
                Glib::ustring::format(refl.GetUInt32(message, &fd));
            return;

        case google::protobuf::FieldDescriptor::CPPTYPE_UINT64:
            row[cols.value] =
                Glib::ustring::format(refl.GetUInt64(message, &fd));
            return;

        case google::protobuf::FieldDescriptor::CPPTYPE_DOUBLE:
            row[cols.value] =
                Glib::ustring::format(refl.GetDouble(message, &fd));
            return;

        case google::protobuf::FieldDescriptor::CPPTYPE_FLOAT:
            row[cols.value] =
                Glib::ustring::format(refl.GetFloat(message, &fd));
            return;
        ...
        case google::protobuf::FieldDescriptor::CPPTYPE_STRING:
            row[cols.value] = build_decoded_tree_format_string(
                refl.GetString(message, &fd), fd.type());
            return;

        case google::protobuf::FieldDescriptor::CPPTYPE_MESSAGE:
            build_decoded_tree_nontoplevel(
                refl.GetMessage(message, &fd), ts, cols, row);
            return;
    }
    std::abort();
}

void build_decoded_tree_field_repeated(
    const google::protobuf::Message &message,
    const google::protobuf::FieldDescriptor &fd, int index,
    Glib::RefPtr<Gtk::TreeStore> ts, PacketDecodedTreeColumns &cols,
    Gtk::TreeRow &row)
{
    const google::protobuf::Reflection &refl = *message.GetReflection();
    ...
    {
        case google::protobuf::FieldDescriptor::CPPTYPE_INT32:
            row[cols.value] = Glib::ustring::format(
                refl.GetRepeatedInt32(message, &fd, index));
            return;

        case google::protobuf::FieldDescriptor::CPPTYPE_INT64:
            row[cols.value] = Glib::ustring::format(
                refl.GetRepeatedInt64(message, &fd, index));
            return;

        case google::protobuf::FieldDescriptor::CPPTYPE_UINT32:
            row[cols.value] = Glib::ustring::format(
                refl.GetRepeatedUInt32(message, &fd, index));
            return;

        case google::protobuf::FieldDescriptor::CPPTYPE_UINT64:
            row[cols.value] = Glib::ustring::format(
                refl.GetRepeatedUInt64(message, &fd, index));
            return;

        case google::protobuf::FieldDescriptor::CPPTYPE_DOUBLE:
            row[cols.value] = Glib::ustring::format(
                refl.GetRepeatedDouble(message, &fd, index));
            return;

        case google::protobuf::FieldDescriptor::CPPTYPE_FLOAT:
            row[cols.value] = Glib::ustring::format(
                refl.GetRepeatedFloat(message, &fd, index));
            return;

        case google::protobuf::FieldDescriptor::CPPTYPE_BOOL:
            row[cols.value] = refl.GetRepeatedBool(message, &fd, index)
                                  ? u8"true"
                                  : u8"false";
            return;
        ...
        case google::protobuf::FieldDescriptor::CPPTYPE_STRING:
            row[cols.value] = build_decoded_tree_format_string(
                refl.GetRepeatedString(message, &fd, index), fd.type());
            return;

        case google::protobuf::FieldDescriptor::CPPTYPE_MESSAGE:
            build_decoded_tree_nontoplevel(
                refl.GetRepeatedMessage(message, &fd, index), ts, cols, row);
            return;
    }
    std::abort();
}

void build_decoded_tree_message(
    const google::protobuf::Message &message, Glib::RefPtr<Gtk::TreeStore> ts,
    PacketDecodedTreeColumns &cols,
    std::function<Gtk::TreeRow(const Glib::ustring &, const Glib::ustring &)>
        append_kv)
{
    ...
    {
        const Log::Parameter &param =
            dynamic_cast<const Log::Parameter &>(message);
        ...
        {
            append_kv(
                param.name(), Glib::ustring::format(param.double_value()));
        }
        else if (param.has_radian_value())
        {
            append_kv(
                param.name(),
                Glib::ustring::compose(u8"%1r", param.radian_value()));
        }
        else if (param.has_degree_value())
        {
            append_kv(
                param.name(),
                Glib::ustring::compose(u8"%1°", param.degree_value()));
        }
        else
        {
            std::abort();
    ...
    for (int i = 0; i < desc.field_count(); ++i)
    {
        ...
        switch (fd.label())
        {
            case google::protobuf::FieldDescriptor::LABEL_OPTIONAL:
                ...
                {
                    build_decoded_tree_field_singular(
                        message, fd, ts, cols, row);
                }
                else
                {
            ...
            case google::protobuf::FieldDescriptor::LABEL_REPEATED:
                ok              = true;
                row[cols.value] = Glib::ustring::compose(
                    u8"%1 instances", refl.FieldSize(message, &fd));
                for (int i = 0; i < refl.FieldSize(message, &fd); ++i)
                {
                    Gtk::TreeRow child = cols.append_kv(
                        ts, row, Glib::ustring::compose(u8"Element %1", i));
                    build_decoded_tree_field_repeated(
                        message, fd, i, ts, cols, child);
                }
                break;
        }
        assert(ok);
...
void build_decoded_tree_toplevel(
    const google::protobuf::Message &message, Glib::RefPtr<Gtk::TreeStore> ts,
    PacketDecodedTreeColumns &cols)
{
    build_decoded_tree_message(
        message, ts, cols,
        [&cols, ts](const Glib::ustring &k, const Glib::ustring &v) {
            return cols.append_kv(ts, k, v);
        });
}
    ...
    PacketDecodedTreeColumns &cols, Gtk::TreeRow parent)
{
    ...
    {
        const Log::MonotonicTimeSpec &mts =
            dynamic_cast<const Log::MonotonicTimeSpec &>(message);
    ...
    else if (dynamic_cast<const Log::Vector2 *>(&message))
    {
        ...
        parent[cols.value]    = Glib::ustring::compose(
            u8"(%1, %2)", v.x() / 1000000.0, v.y() / 1000000.0);
        return;
    }
    else if (dynamic_cast<const Log::Vector3 *>(&message))
    {
        ...
        parent[cols.value]    = Glib::ustring::compose(
            u8"(%1, %2, %3)", v.x() / 1000000.0, v.y() / 1000000.0,
            v.t() / 1000000.0);
        return;
    }
    ...
    build_decoded_tree_message(
        message, ts, cols,
        [&cols, ts, parent](const Glib::ustring &k, const Glib::ustring &v) {
            return cols.append_kv(ts, parent, k, v);
        });
}

void build_decoded_tree(
    const Log::Record &record, Glib::RefPtr<Gtk::TreeStore> ts,
    PacketDecodedTreeColumns &cols)
{
    // We don't want the Record message itself to be shown in the tree as it's
    // only a wrapper and would obfuscate the output.
    ...
    for (int i = 0; i < desc.field_count(); ++i)
    {
        ...
        if (fd.is_optional() &&
            ...
            refl.HasField(record, &fd))
        {
            // This record contains an optional field.
            // The type of the optional field itself is displayed in the list of
            // records.
            // Therefore, there is no need to show a tree node for said field.
            // Instead, the contents of the field's message should become the
            ...
            refl.HasField(record, &fd))
        {
            ...
            Gtk::TreeRow row =
                cols.append_kv(ts, Glib::ustring::compose(u8"Value", i));
            ...
            fd.is_repeated() &&
            fd.type() == google::protobuf::FieldDescriptor::TYPE_MESSAGE &&
            refl.FieldSize(record, &fd))
        {
            // This record contains a repeated field.
            // Show a tree node for each instance.
            for (int i = 0; i < refl.FieldSize(record, &fd); ++i)
            {
                Gtk::TreeRow row = cols.append_kv(
                    ts, Glib::ustring::compose(u8"Element %1", i));
                build_decoded_tree_nontoplevel(
                    refl.GetRepeatedMessage(record, &fd, i), ts, cols, row);
            }
            return;
        }
    }
...
class LogAnalyzer::Impl final : public NonCopyable
{
   public:
    ...
    explicit Impl(const std::string &pathname)
...
LogAnalyzer::LogAnalyzer(Gtk::Window &parent, const std::string &pathname)
    : impl(new Impl(pathname)),
      pane_fixed(false),
      packets_list_view(impl->alm),
      to_tsv_button(u8"To TSV")
{
    set_title(Glib::ustring::compose(
        u8"Thunderbots Log Tools - Analyzer - %1",
    ...
    packets_list_view.get_selection()->signal_changed().connect(sigc::mem_fun(
        this, &LogAnalyzer::on_packets_list_view_selection_changed));
    ...
    packet_decoded_tree.append_column(
        u8"Field", impl->packet_decoded_tree_columns.key);
    packet_decoded_tree.get_column(0)->set_resizable();
    packet_decoded_tree.append_column(
        u8"Value", impl->packet_decoded_tree_columns.value);
    ...
    to_tsv_button.signal_clicked().connect(
        sigc::mem_fun(this, &LogAnalyzer::on_to_tsv_clicked));
...
void LogAnalyzer::on_packets_list_view_selection_changed()
{
    ...
    {
        const Log::Record &record = impl->records[static_cast<std::size_t>(
            (*packets_list_view.get_selection()
                  ->get_selected_rows()
                  .begin())[0])];
        Glib::RefPtr<Gtk::TreeStore> ts =
            Gtk::TreeStore::create(impl->packet_decoded_tree_columns);
...
void LogAnalyzer::on_to_tsv_clicked()
{
    ...
    if (dlg.run() == Gtk::RESPONSE_OK)
    {
        Gtk::FileChooserDialog fc(
            *this, u8"Save to TSV", Gtk::FILE_CHOOSER_ACTION_SAVE);
        fc.set_local_only();
