...
class ParamTreeNode : public NonCopyable
{
   public:
    ...
    /**
     * \brief Constructs a new \c ParamTreeNode that is not yet linked into the
     * tree.
     *
    ...
    /**
     * \brief Finds or creates an internal node that is a descendent of this
     * node.
     *
    ...
    /**
     * \brief Returns a string consisting of slash-separated components
     * constituting the full path to the node.
     *
     * \return the path.
     */
...
/**
 * \brief A generic parameter that can be edited and whose value can be stored
...
class NumericParam : public Param
{
   public:
    ...
    /**
     * \brief Returns the proper number of digits to display after the decimal
     * point when rendering or editing this parameter.
     *
   ...
   protected:
     ...
     *
     * \param[in] integer \c true if the parameter should take integer values,
     * or \c false if it may take non-integer values.
     */
    explicit NumericParam(
...
class IntParam final : public NumericParam
{
   public:
     ...
     */
    explicit IntParam(
        const Glib::ustring &name, const Glib::ustring &location, int def,
        int min, int max);
...
/**
 * \brief A floating-point parameter that can be edited by means of a text
 * field.
 */
