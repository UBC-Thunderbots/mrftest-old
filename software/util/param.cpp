   ...
   public:
        ...
        if (!path.empty())
        {
            if (path[0] == 47 /* slash */)
            {
                throw std::invalid_argument(
                    "Path component in parameter tree path is empty");
            }
            Glib::ustring::size_type index_of_slash =
                path.find(static_cast<gunichar>(47) /* slash */);
                    ...
                    if (!child)
                    {
                        throw std::invalid_argument(
                            Glib::locale_from_utf8(Glib::ustring::compose(
                                u8"Duplicate name “%1” in parameter tree path "
                                u8"“%2” (both parameter and category)",
                                component, this->path())));
                    }
                    break;
                }
            ...
            return child->internal_node(
                index_of_slash != Glib::ustring::npos
                    ? path.substr(index_of_slash + 1)
                    : Glib::ustring());
        }
        else
        {
    ...
    void initialize()
    {
        ...
        std::sort(
            ...
            [](const ParamTreeNode *n1, const ParamTreeNode *n2) {
                return n1->collate_key() < n2->collate_key();
            });

        // The cached collation keys are no longer needed.
        std::for_each(children.begin(), children.end(), [](ParamTreeNode *n) {
            n->collate_key_clear();
        });
        ...
        // Check for duplicates.
        {
            ...
            if (i != children.end())
            {
                throw std::invalid_argument(
                    Glib::locale_from_utf8(Glib::ustring::compose(
                        u8"Duplicate name “%1” in parameter tree path “%2”",
                        (*i)->name(), path())));
            }
        }
        ...
        {
            children[i]->link(
                i, i < (children.size() - 1) ? children[i + 1] : nullptr,
                i ? children[i - 1] : nullptr, this);
    ...
    void load(const xmlpp::Element *elt) override
    {
        ...
        // Make a mapping from an XML child element's name's collation key to
        // the node.
        std::unordered_map<std::string, const xmlpp::Element *>
            child_elts_byname;
        for (const xmlpp::Node *child_node : elt->get_children())
        {
            const xmlpp::Element *child_elt =
                dynamic_cast<const xmlpp::Element *>(child_node);
            if (child_elt)
            {
                const xmlpp::Attribute *name_attr =
                    child_elt->get_attribute(u8"name");
                if (name_attr)
                {
                    child_elts_byname[name_attr->get_value().collate_key()] =
                        child_elt;
                }
            }
        }

        // Go through the child nodes in the parameter tree and apply values to
        // them.
        for (ParamTreeNode *i : children)
        {
   ...
   private:
    class Allocator final : public NonCopyable
    {
       public:
        ...
        {
            std::unique_ptr<ParamTreeInternalNode> p(
                new ParamTreeInternalNode(name));
    ...
    explicit ParamTreeInternalNode(const Glib::ustring &name)
        : ParamTreeNode(name)
    {
    }
...
ParamTreeNode::ParamTreeNode(const Glib::ustring &name)
    : name_(name),
      ...
      parent_(nullptr)
{
    ...
    {
        throw std::invalid_argument(
            "Illegal parameter name (must be nonempty)");
    }
    if (exists(name_.begin(), name_.end(), '/'))
    {
        throw std::invalid_argument(
            Glib::locale_from_utf8(Glib::ustring::compose(
                u8"Illegal parameter name “%1” (must not contain a slash)",
                name_)));
    }
}
...
void ParamTreeNode::link(
    std::size_t index, ParamTreeNode *next, ParamTreeNode *prev,
...
Param::Param(const Glib::ustring &name, const Glib::ustring &location)
    : ParamTreeNode(name)
{
    assert(location[0]);
    ParamTreeInternalNode *parent =
        ParamTreeNode::root()->internal_node(location);
...
BoolParam::BoolParam(
    const Glib::ustring &name, const Glib::ustring &location, bool def)
    : Param(name, location), value_(def), default_(def)
{
}
...
void BoolParam::load(const xmlpp::Element *elt)
{
    if (elt->get_name() == u8"boolean")
    {
        ...
        {
            value_ =
                text_node->get_content().find(u8"true") != Glib::ustring::npos;
        }
    }
}
...
NumericParam::NumericParam(
    const Glib::ustring &name, const Glib::ustring &location, double def,
      ...
      adjustment_()
{
    if (!(min <= def && def <= max))
    {
        throw std::invalid_argument(
            Glib::locale_from_utf8(Glib::ustring::compose(
                u8"Parameter default value for %1/%2 out of valid range.",
                location, name)));
    }
}
...
{
    Param::initialize();
    adjustment_ = Gtk::Adjustment::create(
        def, min, max, integer ? 1 : compute_step(min, max),
        integer ? 10 : compute_page(min, max), 0);
    adjustment_->signal_value_changed().connect(
        signal_changed_reflector.make_slot());
}

IntParam::IntParam(
    const Glib::ustring &name, const Glib::ustring &location, int def, int min,
    int max)
    : NumericParam(name, location, def, min, max, true)
{
}
...
void IntParam::load(const xmlpp::Element *elt)
{
    if (elt->get_name() == u8"integer")
    {
        ...
        adjustment()->set_value(
            std::stoi(ustring2wstring(text_node->get_content())));
    }
}
...
DoubleParam::DoubleParam(
    ...
    : NumericParam(name, location, def, min, max, false)
{
}
...
void DoubleParam::load(const xmlpp::Element *elt)
{
    if (elt->get_name() == u8"double")
    {
        const xmlpp::TextNode *text_node = elt->get_child_text();
        adjustment()->set_value(
            std::stod(ustring2wstring(text_node->get_content())));
    }
}
