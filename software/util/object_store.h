...
/**
 * \brief An ObjectStore allows multiple clients to store arbitrary objects
 * attached to another object, without the holder knowing a priori the details
 * of the stored objects, and without multiple clients overwriting each others’
 * stored objects.
 */
class ObjectStore final : public NonCopyable
{
   public:
    ...
    /**
     * \brief Fetches an object from the ObjectStore, creating it if it does not
     * yet exist.
     *
     * \param[in] tid the key identifying the specific client (generally
     * <code>typeid(*this)</code> in the client)
     *
