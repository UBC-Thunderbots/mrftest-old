...
inline timespec timespec_add(const timespec &ts1, const timespec &ts2) {
	timespec result;
	timespec_add(ts1, ts2, result);
	return result;
}

inline void timespec_sub(const timespec &ts1, const timespec &ts2, timespec &result) {
...
inline timespec timespec_sub(const timespec &ts1, const timespec &ts2) {
	timespec result;
	timespec_sub(ts1, ts2, result);
	return result;
}

inline int timespec_cmp(const timespec &ts1, const timespec &ts2) {
...
inline timespec double_to_timespec(double seconds) {
	timespec ts;
...
inline unsigned int timespec_to_millis(const timespec &ts) {
	return static_cast<unsigned int>(ts.tv_sec * 1000U + ts.tv_nsec / 1000000U);
}

inline unsigned int timespec_to_nanos(const timespec &ts) {
	return static_cast<unsigned int>(ts.tv_sec * 1000000000U + ts.tv_nsec);
}

inline Glib::ustring timespec_to_string(const timespec &ts) {
	if (ts.tv_sec >= 0) {
...
inline Glib::ustring timespec_to_string_machine(const timespec &ts) {
	if (ts.tv_sec >= 0) {
