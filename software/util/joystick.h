 ...
 *
 * Two identifiers are considered equal if their ID numbers are equal, ignoring their names.
 *
 ...
 */
inline bool operator==(const Joystick::Identifier &x, const Joystick::Identifier &y) {
	return x.bus_type == y.bus_type && x.vendor_id == y.vendor_id && x.product_id == y.product_id && x.version == y.version;
}
