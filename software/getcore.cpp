...
void run_robot(const std::string &filename)
{
    ...
    // Move the robot into configuration 1 (it will nearly always already be
    // there).
    ...
    for (uint32_t pointer = 0U; pointer < CORE_SIZE; pointer += BLOCK_SIZE)
    {
        ...
        devh.control_in(
            ...
            ROBOT_READ_CORE, static_cast<uint16_t>(pointer / BLOCK_SIZE), 0,
            buffer, BLOCK_SIZE, 0);
    ...
    std::cout.flush();
    {
        ...
        devh.control_in(
            LIBUSB_REQUEST_TYPE_VENDOR | LIBUSB_RECIPIENT_DEVICE,
            ROBOT_READ_BUILD_ID, 0, 0, &id, sizeof(id), 0);
...
void run_dongle(const std::string &filename)
{
    ...
    for (uint32_t pointer = 0U; pointer < CORE_SIZE; pointer += BLOCK_SIZE)
    {
        ...
        devh.control_in(
            LIBUSB_REQUEST_TYPE_VENDOR | LIBUSB_RECIPIENT_DEVICE,
            MRF::CONTROL_REQUEST_READ_CORE,
            static_cast<uint16_t>(pointer / BLOCK_SIZE), 0, buffer, BLOCK_SIZE,
            0);
        ofs.write(buffer, BLOCK_SIZE);
    }
    ofs.close();
    ...
    std::cout.flush();
    {
        ...
        devh.control_in(
            LIBUSB_REQUEST_TYPE_VENDOR | LIBUSB_RECIPIENT_DEVICE,
            MRF::CONTROL_REQUEST_READ_BUILD_ID, 0, 0, &id, sizeof(id), 0);
        std::cout << "0x" << tohex(id, 8) << '\n';
    }
}
}
