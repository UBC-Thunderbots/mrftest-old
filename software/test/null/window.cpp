...
class NullTesterWindow::MappedJoysticksModel final : public AbstractListModel, public Glib::Object {
	public:
		...
		joystick_chooser(Glib::RefPtr<Gtk::TreeModel>::cast_static(mapped_joysticks))
{
	...
	joystick_chooser.signal_changed().connect(sigc::mem_fun(this, &NullTesterWindow::on_joystick_chooser_changed));
	...
	Gtk::Main::signal_key_snooper().connect(sigc::mem_fun(this, &NullTesterWindow::key_snoop));
...
void NullTesterWindow::on_joystick_chooser_changed() {
	...
	if (pstick) {
		...
		for (unsigned int i = 0; i < JoystickMapping::N_AXES; ++i) {
			if (m.has_axis(i)) {
				joystick_signal_connections.push_back(stick.axes()[m.axis(i)].signal_changed().connect(sigc::mem_fun(this, &NullTesterWindow::on_joystick_drive_axis_changed)));
			}
		}
		if (m.has_button(JoystickMapping::BUTTON_DRIBBLE)) {
			joystick_signal_connections.push_back(stick.buttons()[m.button(JoystickMapping::BUTTON_DRIBBLE)].signal_changed().connect(sigc::mem_fun(this, &NullTesterWindow::on_joystick_dribble_changed)));
		}
		if (m.has_button(JoystickMapping::BUTTON_KICK)) {
			joystick_signal_connections.push_back(stick.buttons()[m.button(JoystickMapping::BUTTON_KICK)].signal_changed().connect(sigc::mem_fun(this, &NullTesterWindow::on_joystick_kick_changed)));
		}
		if (m.has_button(JoystickMapping::BUTTON_SCRAM)) {
			joystick_signal_connections.push_back(stick.buttons()[m.button(JoystickMapping::BUTTON_SCRAM)].signal_changed().connect(sigc::mem_fun(this, &NullTesterWindow::on_joystick_scram_changed)));
		}
