...
TesterWindow::TesterWindow(MRFDongle &dongle, MRFRobot &robot) : mapped_joysticks(MappedJoysticksModel::create()), robot(robot), feedback_frame(u8"Feedback"), feedback_panel(dongle, robot), drive_frame(u8"Drive"), drive_panel(robot), dribble_button(u8"Dribble"), kicker_frame(u8"Kicker"), kicker_panel(robot), leds_frame(u8"LEDs"), leds_panel(dongle, robot.index), params_frame(u8"Parameters"), params_panel(dongle, robot), power_frame(u8"Power"), power_panel(dongle, robot.index), joystick_frame(u8"Joystick"), joystick_sensitivity_high_button(joystick_sensitivity_group, u8"High Sensitivity"), joystick_sensitivity_low_button(joystick_sensitivity_group, u8"Low Sensitivity"), joystick_chooser(Glib::RefPtr<Gtk::TreeModel>::cast_static(mapped_joysticks)) {
