...
const LEDMode MODES[] = {
    {u8"Normal operation", 0x20},      {u8"Lamp test", 0x21},
    {u8"Wheel 0 Hall sensors", 0x00},  {u8"Wheel 1 Hall sensors", 0x01},
    {u8"Wheel 2 Hall sensors", 0x02},  {u8"Wheel 3 Hall sensors", 0x03},
    {u8"Dribbler Hall sensors", 0x04}, {u8"Wheel 0 encoders", 0x05},
    {u8"Wheel 1 encoders", 0x06},      {u8"Wheel 2 encoders", 0x07},
    {u8"Wheel 3 encoders", 0x08},
};
}

LEDsPanel::LEDsPanel(MRFDongle &dongle, unsigned int index)
    : dongle(dongle), index(index), set_button(u8"Set")
{
    ...
    set_button.signal_clicked().connect(
        sigc::mem_fun(this, &LEDsPanel::set_mode));
...
void LEDsPanel::set_mode()
{
    if (mode_chooser.get_active_row_number() >= 0)
    {
        ...
        message.reset(new MRFDongle::SendReliableMessageOperation(
            dongle, index, 3, data, sizeof(data)));
        message->signal_done.connect(
            sigc::mem_fun(this, &LEDsPanel::check_result));
    }
}
...
void LEDsPanel::check_result(AsyncOperation<void> &op)
{
    ...
    catch (const MRFDongle::SendReliableMessageOperation::NotAssociatedError &)
    {
        ...
        const MRFDongle::SendReliableMessageOperation::NotAcknowledgedError &)
    {
