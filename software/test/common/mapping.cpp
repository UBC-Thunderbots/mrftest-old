...
const char *const BUTTON_XML_NAMES[] = {
    u8"dribble", u8"kick", u8"scram",
};
static_assert(
    G_N_ELEMENTS(BUTTON_XML_NAMES) == JoystickMapping::N_BUTTONS,
    "BUTTON_XML_NAMES array must be same size as N_BUTTONS enumeration "
    "element");
}
...
static_assert(
    G_N_ELEMENTS(JoystickMapping::BUTTON_LABELS) == JoystickMapping::N_BUTTONS,
    u8"BUTTON_LABELS array must be same size as N_BUTTONS enumeration element");

constexpr unsigned int JoystickMapping::N_AXES;

JoystickMapping::JoystickMapping(const Joystick::Identifier &identifier)
    : identifier_(identifier), name_collate(identifier_.name.collate_key())
{
		...
		name_collate(identifier_.name.collate_key())
{
    ...
    {
        const xmlpp::Element *group_elt =
            dynamic_cast<const xmlpp::Element *>(group_elt_node);
        if (group_elt)
        {
            int *mappings;
            ...
            const xmlpp::Node::NodeList &mapping_elts =
                group_elt->get_children();
            for (const xmlpp::Node *mapping_elt_node : mapping_elts)
            {
                const xmlpp::Element *mapping_elt =
                    dynamic_cast<const xmlpp::Element *>(mapping_elt_node);
                if (mapping_elt)
                {
                    assert(mapping_elt->get_name() == u8"mapping");
                    const Glib::ustring &logs =
                        mapping_elt->get_attribute_value(u8"logical");
                    const Glib::ustring &physs =
                        mapping_elt->get_attribute_value(u8"physical");
                    assert(!logs.empty());
                    assert(!physs.empty());
                    int log;
                    if (xml_names)
                    {
                        for (log = 0;
                             static_cast<unsigned int>(log) < n_mappings &&
                             logs != xml_names[log];
                             ++log)
                        {
                        }
    ...
    for (std::size_t i = 0; i < G_N_ELEMENTS(groups); ++i)
    {
        ...
        for (unsigned int j = 0; j < groups[i].n_mappings; ++j)
        {
            if (groups[i].mappings[j] >= 0)
            {
                ...
                mapping_elt->set_attribute(
                    u8"logical",
                    groups[i].xml_names ? groups[i].xml_names[j] : todecu(j));
                mapping_elt->set_attribute(
                    u8"physical",
                    todecu(static_cast<unsigned int>(groups[i].mappings[j])));
            }
        }
    }
}
