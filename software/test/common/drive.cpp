...
namespace {
	...
	void on_update_matrix(Drive::Robot &bot, const double sliders[4], bool) {
		bot.direct_velocity({sliders[0], sliders[1]}, Angle::of_degrees(sliders[2]));
	}
	...
	void on_execute_shoot_kick(Drive::Robot &bot, const double sliders[4], bool exact_angle) {
		if (exact_angle) {
			bot.move_shoot({sliders[0], sliders[1]}, Angle::of_degrees(sliders[2]), sliders[3], false);
		} else {
			bot.move_shoot({sliders[0], sliders[1]}, sliders[3], false);
		}
	}

	void on_execute_shoot_chip(Drive::Robot &bot, const double sliders[4], bool exact_angle) {
		if (exact_angle) {
			bot.move_shoot({sliders[0], sliders[1]}, Angle::of_degrees(sliders[2]), sliders[3], true);
		} else {
	...
	void on_execute_pivot(Drive::Robot &bot, const double sliders[4], bool) {
		bot.move_pivot({sliders[0], sliders[1]}, Angle::of_degrees(sliders[2]), Angle::of_degrees(sliders[3]));
	}
	...
	struct Mode final {
		...
		void (*on_update_or_execute)(Drive::Robot &bot, const double sliders[4], bool checkbox);
...
DrivePanel::DrivePanel(Drive::Robot &robot) :
	...
	for (unsigned int i = 0; i != G_N_ELEMENTS(scales); ++i) {
		...
		scales[i].signal_value_changed().connect(sigc::mem_fun(this, &DrivePanel::on_update));
		scales_table.attach(scales_labels[i], 0, 1, i, i + 1, Gtk::FILL, Gtk::FILL);
		scales_table.attach(scales[i], 1, 2, i, i + 1, Gtk::EXPAND | Gtk::FILL, Gtk::FILL);
	}
	pack_start(scales_table, Gtk::PACK_SHRINK);
	misc_checkbox.set_sensitive(false);
	misc_checkbox.signal_toggled().connect(sigc::mem_fun(this, &DrivePanel::on_update));
	pack_start(misc_checkbox, Gtk::PACK_SHRINK);
	execute_button.signal_clicked().connect(sigc::mem_fun(this, &DrivePanel::on_execute));
...
void DrivePanel::get_low_sensitivity_scale_factors(double scale[4]) {
	...
	if (row >= 0) {
		void (*fn)(double scale[4]) = MODES[row].get_low_sensitivity_scale_factors;
		if (fn) {
			fn(scale);
		} else {
...
void DrivePanel::on_mode_changed() {
	...
	if (row >= 0) {
		...
		for (unsigned int i = 0; i < G_N_ELEMENTS(scales); ++i) {
			...
			scales[i].get_adjustment()->configure(info.value, info.min, info.max, info.step, info.page, 0);
...
void DrivePanel::on_execute() {
	...
	if (row >= 0) {
		...
		MODES[row].on_update_or_execute(robot, values, misc_checkbox.get_active());
	}
}
