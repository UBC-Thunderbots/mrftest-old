...
namespace {
	...
	class SimEngine : public SimulatorEngine {
		private:
			RefPtr<BallODE> the_ball;
			std::vector<RefPtr<PlayerODE> > the_players;
			RefPtr<PlayerODE> emptyPlayer;
		public:
			double timeStep;
			dWorldID eworld;
			dSpaceID space;
			...
			SimEngine(){
				...
				RefPtr<BallODE> b(new BallODE(eworld, space));
				the_ball = b;
			...
			RefPtr<SimulatorBall> get_ball() {
				return the_ball;
			}

			RefPtr<SimulatorPlayer> add_player() {
				RefPtr<PlayerODE>	 p(new PlayerODE(eworld, space, the_ball->ballGeom, static_cast<double>(UPDATES_PER_TICK)));
			...
			RefPtr<PlayerODE> get_player_from_shape(dGeomID shape){
				for (unsigned int i = 0; i < the_players.size(); i++) {
			...
			RefPtr<PlayerODE> get_player_from_shape_ground(dGeomID shape){
				for (unsigned int i = 0; i < the_players.size(); i++) {
			...
			void remove_player(RefPtr<SimulatorPlayer> p) {
				for (unsigned int i = 0; i < the_players.size(); i++) {
					if (RefPtr<SimulatorPlayer>::cast_static(the_players[i]) == p) {
			...
			void handleBallCollisionWithGround(dGeomID o1, dGeomID o2){
				...
				double frict = MU*6;
				int i=0;
				 RefPtr<PlayerODE> robot = emptyPlayer;
			...
			void handleBallCollision (dGeomID o1, dGeomID o2){
				  ...
				  RefPtr<PlayerODE> robot1 = get_player_from_shape(o1);
				  RefPtr<PlayerODE> robot2 = get_player_from_shape(o2);
			...
			void handleRobotBallCollision (dGeomID o1, dGeomID o2){
				int i=0;
				  ...
				  RefPtr<PlayerODE> robot = get_player_from_shape(o1);
	...
	class SimEngineFactory : public SimulatorEngineFactory {
		public:
			...
			RefPtr<SimulatorEngine> create_engine() {
				RefPtr<SimulatorEngine> p(new SimEngine);
				return p;
			}
	};
