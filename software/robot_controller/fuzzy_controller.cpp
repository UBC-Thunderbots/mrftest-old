...
void fuzzy_controller::move(const point &new_position, double new_orientation, point &linear_velocity, double &angular_velocity) {
	...
	double distance_factor = (new_position - current_position).len() / 0.6; // <-- this is a parameter which can be tuned
	...
	if (distance_factor < 0.4) distance_factor = 0.4; // <-- this is a parameter which can be tuned
