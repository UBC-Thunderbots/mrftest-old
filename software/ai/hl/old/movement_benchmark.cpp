...
namespace {
	...
	void MovementBenchmarkStrategy::play() {
		...
		if (friendly.size() > 1) {
			friendly.get(1)->move(Point(0.5, 0), 0, 0, AI::Flags::MoveType::NORMAL, AI::Flags::MovePrio::HIGH);
		}
		if (friendly.size() > 2) {
			friendly.get(2)->move(Point(-0.5, 0), 0, 0, AI::Flags::MoveType::NORMAL, AI::Flags::MovePrio::HIGH);
		}
	}
