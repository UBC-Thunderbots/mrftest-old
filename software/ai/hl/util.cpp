...
bool AI::HL::Util::point_in_friendly_defense(const Field &field, const Point p)
{
    ...
    const double defense_width   = field.defense_area_width();
    const Point friendly_goal    = field.friendly_goal();

    return (
        p.x > friendly_goal.x && p.x < friendly_goal.x + defense_width &&
        p.y > -defense_stretch / 2 && p.y < defense_stretch / 2);
}
...
bool AI::HL::Util::path_check(
    const Point &begin, const Point &end, const std::vector<Robot> &robots,
    const double thresh)
{
    const Point direction = (end - begin).norm();
    const double dist     = (end - begin).len();
...
std::pair<Point, Angle> AI::HL::Util::calc_best_shot(
...
std::vector<std::pair<Point, Angle>> AI::HL::Util::calc_best_shot_all(
    const Field &f, const std::vector<Point> &obstacles, const Point &p,
    const double radius)
{
    const Point p1 = Point(f.length() / 2.0, -f.goal_width() / 2.0);
...
std::pair<Point, Angle> AI::HL::Util::calc_best_shot(
    World world, const Player player, const double radius)
{
