...
bool AI::HL::STP::Action::goalie_rush(const World &world, Player::Ptr player, Point & rushpos) {
	...
	return ballvel.len() > ball_dangerous_speed && ballvel.x < -1e-6 && (std::min(std::fabs(goalpos.y),std::fabs(rushpos.y)) < world.field().goal_width()/2.0);
}
...
void AI::HL::STP::Action::lone_goalie(const World &world, Player::Ptr player) {
	...
	Point rushpos;
	if (goalie_rush(world, player, rushpos)){
