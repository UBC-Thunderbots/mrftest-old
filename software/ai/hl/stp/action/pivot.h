...
inline void waitForPivot(caller_t& ca, Point destination, Player player)
{
    ...
    do
    {
        playerDestinationDifference =
            player.position() - destination;  // calculates the vector between
                                              // the player and their
                                              // destination
        differenceMagnitude = sqrt(
            pow(playerDestinationDifference.x, 2.0) +
            pow(playerDestinationDifference.y,
                2.0));  // converts this difference vector to a magnitude
        Action::yield(ca);
    } while (differenceMagnitude > tolerence);  // compares the distance betweem
                                                // the robot and its destination
                                                // to the specified tolerence
}

inline void waitForPivot(
    caller_t& ca, Point destination, Player player, double tolerence)
{
    Point playerDestinationDifference;
    double differenceMagnitude;
