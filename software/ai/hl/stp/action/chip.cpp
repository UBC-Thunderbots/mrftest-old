...
#warning "Does pivot work properly?"
    ...
    *	hitting directly on chipper. This results in the player continously
    *ramming
    *	the ball, which is bad for kickoffs. The following code will be
    *uncommented
    ...
    */
    ...
    {
        //		// angle is right but chicker not ready, ram the ball
        // and
        // get closer to
        //		// target, only use in normal play
        //		if (world.playtype() == AI::Common::PlayType::PLAY) {
        //			const Point diff = world.ball().position() -
        // player.position();
    ...
    Action::shoot_target(
        ca, world, player,
        Point::of_angle((target - world.ball().position()).orientation())
            .norm(10),
        (target - player.position()).len(), true);
    LOG_DEBUG(u8"autochip, Action::chip_target");

    return true;
}

bool AI::HL::STP::Action::chip_at_location(
    caller_t& ca, World world, Player player, const Point location_to_chip_at,
    double chip_distance, Angle chip_orientation)
{
    Action::shoot_target(
        ca, world, player, location_to_chip_at, chip_distance, true);
    return true;
}
