...
//	// set the destination point to be just behind the ball in the correct
// direction at the offset distance
//	Point dest = -(target - world.ball().position()).norm() * radius +
// world.ball().position();
//	//player.prio(AI::Flags::MovePrio::HIGH);
//
//	Primitive prim = Primitives::Pivot(player, dest, swing,
//(world.ball().position() - dest).orientation());
...
#warning RoboCup 2017 hack. Call moves around a point to simulate pivot. This should use the pivot primitive once it works
    ...
    else
    {
        ...
        bool clockwise = is_clockwise(
            (player.position() - target), Point::of_angle(finalAngle));
        if ((player.position() - target).orientation().angle_diff(finalAngle) <=
            Angle::of_degrees(ROTATION))
        {
        ...
        else
        {
            if (clockwise)
            {
                dest = (player.position() - target)
                           .norm(radius)
                           .rotate(Angle::of_degrees(-ROTATION));
            }
            else
            {
                dest = (player.position() - target)
