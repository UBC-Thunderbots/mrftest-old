...
DoubleParam corner_repel_speed(
    u8"speed that repel will be kicking at in a corner",
    u8"AI/HL/STP/Action/repel", 3.0, 1.0, 8.0);
}
...
bool AI::HL::STP::Action::repel(caller_t& ca, World world, Player player)
{
    ...
    {
        Point dest = ball;
        if (dest.x < f.friendly_goal().x + Robot::MAX_RADIUS)
        ...
        player.position().orientation().to_radians() > -M_PI / 2)
    {
        ...
        Action::shoot_target(
            ca, world, player, Point::of_angle(player.orientation()).norm(10),
            BALL_MAX_SPEED, false);
        kicked = true;
    }
    // if we catch the ball and we're facing our own goal, we rotate to the
    // point to the other side
    // rotating towards 0 degrees is okay, because by the time the next tick
    // comes, this case will be false anyways.
    else
    {
        Action::pivot(
            ca, world, player, world.ball().position(), Angle::zero());
        Action::dribble(ca, world, player, player.position(), Angle::zero());
    }

    Action::move(
        ca, world, player, world.ball().position(), diff.orientation());
...
bool AI::HL::STP::Action::corner_repel(caller_t& ca, World world, Player player)
{
    ...
    if (!Predicates::ball_in_our_corner(world) ||
    ...
    // check circle in the middle and the centre line and find the best open
    // spot to shoot at
    const Point p1(0.0, -f.centre_circle_radius()),
        p2(0.0, f.centre_circle_radius());
    std::pair<Point, Angle> centre_circle = angle_sweep_circles(
        player.position(), p1, p2, obstacles, Robot::MAX_RADIUS);

    const Point p3(0.0, -f.width() / 2.0), p4(0.0, f.width() / 2.0);
    std::pair<Point, Angle> centre_line = angle_sweep_circles(
        player.position(), p3, p4, obstacles, Robot::MAX_RADIUS);

    if (centre_circle.second > shoot_accuracy)
    {
        Action::shoot_target(
            ca, world, player, centre_circle.first, corner_repel_speed, false);
        return true;
    }
...
bool AI::HL::STP::Action::goalie_repel(caller_t& ca, World world, Player player)
{
    ...
    double r = world.field().defense_area_radius() +
               world.field().defense_area_stretch() / 2;
    // const Point destination = world.field().friendly_goal() + (ball -
    // world.field().friendly_goal()).norm() * r;
    ...
    if (!player.has_ball())
    {
        ...
        if ((world.ball().position() - world.field().friendly_goal()).len() > r)
        {
            dest = world.field().friendly_goal() +
                   (ball - world.field().friendly_goal()).norm() * r;
        }
        ...
        player.orientation().angle_mod().to_radians() > -M_PI / 2)
    {
        ...
        // chip in whatever direction goalie is facing as long as its not facing
        // back (3 was chosen arbitrarily)
        ...
        AI::HL::STP::Action::shoot_target(
            ca, world, player, shoot_pos, 4, true);
        kicked = true;
        return true;
    }
    // if we catch the ball and we're facing our own goal, we rotate to the
    ...
    {
        Point dest2;
        if (world.ball().position().x < f.friendly_goal().x + Robot::MAX_RADIUS)
        {  // avoid going inside the goal
        ...
        if ((world.ball().position() - world.field().friendly_goal()).len() > r)
        {
            dest2 = world.field().friendly_goal() +
                    (ball - world.field().friendly_goal()).norm() * r;
        }
