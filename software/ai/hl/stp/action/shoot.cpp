...
{
    double playerproj = Geom::proj_len(
        Geom::Seg(world.ball().position(), target), player.position());
    double behinddist = Robot::MAX_RADIUS + 0.18;
    Point behind_ball = world.ball().position() +
                        (world.ball().position() - target).norm(behinddist);

    if (playerproj > -behinddist + 0.10)
    {
        Point dest1 = behind_ball +
                      (world.ball().position() - target)
                          .norm(0.3)
                          .rotate(Angle::quarter());
        Point dest2 = behind_ball +
                      (world.ball().position() - target)
                          .norm(0.3)
                          .rotate(-Angle::quarter());

        // Go to the closest position to the player out of the two sideways
        // points from the behind-ball point
        Point dest = (dest1 - player.position()).lensq() >
                             (dest2 - player.position()).lensq()
                         ? dest2
                         : dest1;
    ...
    else
    {
        ...
        player.move_move(
            dest, (target - world.ball().position()).orientation());
    }
}

void AI::HL::STP::Action::shoot_goal(
    caller_t& ca, World world, Player player, bool chip)
{
    ...
    bool chip)
{
    ...
    // player.unset_flags(AI::Flags::MoveFlags::AVOID_BALL_MEDIUM |
    ...
    caller_t& ca, World world, Player player, bool chip)
{
    ...
    while (true)
    {
        ...
        catch_and_shoot_target(
            ca, world, player, shoot_data.target, BALL_MAX_SPEED, chip);
    }
}
