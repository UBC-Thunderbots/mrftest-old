    ...
    caller_t& ca, World world, Player player, Point target)
{
    ...
    do
    {
        ...
        if (world.ball().velocity().lensq() < 0.05)
        {
            ...
            catch_pos = world.ball().position() +
                        target_line.norm(Robot::MAX_RADIUS * 3);
            catch_orientation = (Evaluation::get_best_shot(world, player) -
                                 world.ball().position())
                                    .orientation();
        }
        else
        {
            ...
            // catch_pos =
            // Evaluation::baller_catch_position(world,
            // player);
            ...
            // catch_pos = Evaluation::quickest_intercept_position(world,
            // player);
        }
        ...
        Action::yield(ca);
        LOGF_INFO(
            "%1, %2, %3", (player.position() - catch_pos).lensq() > 0.08 * 0.08,
            player.velocity().lensq() > 0.07 * 0.07,
            player.orientation().angle_diff(catch_orientation).abs() >
                Angle::of_degrees(5));
    } while ((player.position() - catch_pos).lensq() > 0.08 * 0.08 ||
             player.velocity().lensq() > 0.07 * 0.07 ||
             player.orientation().angle_diff(catch_orientation).abs() >
                 Angle::of_degrees(8));
    ...
    //			| AI::Flags::MoveFlags::AVOID_ENEMY_DEFENSE |
    // AI::Flags::MoveFlags::CAREFUL);
    //
    //    catch_pos = Evaluation::baller_catch_position(world, player);
    //    LOGF_INFO(u8"catching dribble to %1", catch_pos);
    //    Action::move_slp(ca, world, player, catch_pos,
