...
BoolParam DRAW_FRIENDLY_PASS(
    u8"Draw friendly pass lines", u8"AI/HL/STP/UI", false);
...
void AI::HL::STP::draw_player_status(
    World world, Cairo::RefPtr<Cairo::Context> ctx)
{
    for (const Player player : world.friendly_team())
    {
        ...
        ctx->arc(
            player.position().x, player.position().y, Robot::MAX_RADIUS * 0.75,
            0.0, 2 * M_PI);
        ctx->stroke();
    }
}

void AI::HL::STP::draw_friendly_pass(
    World world, Cairo::RefPtr<Cairo::Context> ctx)
{
    if (!DRAW_FRIENDLY_PASS)
        return;
    if (Predicates::our_ball(world))
    {
        for (const Player i : world.friendly_team())
        {
            if (Evaluation::passee_suitable(world, i))
            {
                ...
                ctx->line_to(
                    world.ball().position().x, world.ball().position().y);
                ctx->stroke();
            }
        }
    }
...
void AI::HL::STP::draw_enemy_pass(
    World world, Cairo::RefPtr<Cairo::Context> ctx)
{
    if (!DRAW_ENEMY_PASS)
        return;
    ...
    for (std::size_t i = 0; i < enemy.size(); ++i)
    {
        ...
        if (threats[i].passes_goal > 2)
        {
            ...
            ctx->arc(
                ...
                Robot::MAX_RADIUS + 0.01, 0.0, 2 * M_PI);
            ctx->fill();
            ctx->stroke();
        }
...
void AI::HL::STP::draw_velocity(World world, Cairo::RefPtr<Cairo::Context> ctx)
{
    ...
    for (const Player player : world.friendly_team())
    {
        ...
        ctx->arc(
            player.position().x, player.position().y, vel_mag, vel_direction,
            vel_direction + 1.0);
        ctx->stroke();
    }
}
...
void AI::HL::STP::draw_baller(World world, Cairo::RefPtr<Cairo::Context> ctx)
{
    ...
    if (robot && !Evaluation::possess_ball(world, robot))
    {
        ...
        Evaluation::calc_fastest_grab_ball_dest(
            world.ball().position(), world.ball().velocity(), robot.position(),
            dest);
        ctx->set_source_rgba(0.0, 0.0, 0.0, 0.2);
        ctx->set_line_width(0.01);
        ctx->move_to(robot.position().x, robot.position().y);
