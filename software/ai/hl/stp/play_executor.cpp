...
BoolParam high_priority_always(
    u8"If higher priority play exists, switch", u8"AI/HL/STP/PlayExecutor",
    true);
IntParam playbook_index(
    u8"Current Playbook, use bitwise operations", u8"AI/HL/STP/PlayExecutor", 0,
    0, 9);
}
...
PlayExecutor::PlayExecutor(World w) : world(w), curr_play(nullptr)
{
    ...
    world.friendly_team().signal_membership_changed().connect(
        sigc::mem_fun(this, &PlayExecutor::clear_assignments));
}
...
void PlayExecutor::calc_play()
{
    ...
    LOG_INFO(Glib::ustring::compose(
        u8"Play chosen: %1", curr_play->factory().name()));
}
...
void PlayExecutor::tick()
{
    ...
    if (curr_play)
    {
        ...
        {
            LOG_INFO(Glib::ustring::compose(
                u8"%1: play tactic failed", curr_play->factory().name()));
            curr_play = nullptr;
        }
    }
...
Glib::ustring PlayExecutor::info() const
{
    ...
    {
        text +=
            Glib::ustring::compose(u8"play: %1", curr_play->factory().name());
        for (std::size_t i = 0; i < TEAM_MAX_SIZE; ++i)
        {
            ...
            text += Glib::ustring::compose(
                u8"\n%1: %2", curr_play->get_assignment()[i].pattern(),
                curr_play->get_tactics()[i]->description());
        }
    }
    else
    {
...
{
    const bool enabled[] = {
        enable0, enable1, enable2, enable3, enable4,  enable5,
        enable6, enable7, enable8, enable9, enable10, enable11,
    };
