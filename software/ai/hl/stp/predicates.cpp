...
IntParam shoot_anyway(
    u8"randomize factor that the baller will shoot even if blocked",
    u8"AI/HL/STP/predicates", 5, 1, 10);

DoubleParam near_thresh(
    u8"enemy avoidance distance (robot radius)", u8"AI/HL/STP/predicates", 3.0,
    1.0, 10.0);
DoubleParam chip_estimate(
    u8"estimate of our chip distance (m)", u8"AI/HL/STP/predicates", 1.0, 0.0,
    3.0);
DoubleParam fight_thresh(
    u8"dist thresh to start fight ball with enemy (robot radius)",
    u8"AI/HL/STP/predicates", 2.0, 0.1, 4.0);
DoubleParam looseBall_thresh(
    u8"distance to trigger loose_ball predicate", u8"AI/HL/STP/predicates", 1.0,
    0.5, 6.0);
...
bool Predicates::TheirTeamSizeAtLeast::compute(
    World world, const unsigned int n)
{
...
{
    return ball_on_our_side(world) && !ball_in_our_corner(world) &&
           !ball_midfield(world);
}
...
{
    return ball_on_their_side(world) && !ball_in_their_corner(world) &&
           !ball_midfield(world);
}
...
bool Predicates::BallerCanChip::compute(World world, bool towardsEnemy)
{
    ...
    if ((fabs(world.ball().position().x) < chip_estimate) &&
        (ball_on_our_side(world) == towardsEnemy))
    {
        return false;
    }
    ...
    for (const Robot i : world.enemy_team())
    {
        if ((baller.position() - i.position()).len() <=
            near_thresh * AI::HL::W::Robot::MAX_RADIUS)
        {
            return false;
        }
    }
...
bool Predicates::EnemyBallerCanPassShoot::compute(World world)
{
    ...
    return Evaluation::calc_enemy_pass(world, baller) > 0 &&
           Evaluation::calc_enemy_pass(world, baller) < 3;
}
...
bool Predicates::NumOfEnemiesOnOurSideAtLeast::compute(
    World world, const unsigned int n)
{
    unsigned int cnt = 0;
    for (const Robot i : world.enemy_team())
...
Predicates::NumOfEnemiesOnOurSideAtLeast
    Predicates::num_of_enemies_on_our_side_at_least;
...
bool Predicates::FightBall::compute(World world)
{
    ...
    else
    {
        ...
        return (friendly_baller && enemy_baller) &&
               (friendly_baller.position() - world.ball().position()).len() <
                   fight_thresh * Robot::MAX_RADIUS &&
               (enemy_baller.position() - world.ball().position()).len() <
                   fight_thresh * Robot::MAX_RADIUS;
    }
}
...
bool Predicates::LooseBall::compute(World world)
{
    ...
    for (const Robot i : world.enemy_team())
    {
        if (dist < (i.position() - ball).len())
            dist = (i.position() - ball).len();
    }
    for (const Robot i : world.friendly_team())
    {
        if (dist - (i.position() - ball).len() > looseBall_thresh)
            return true;
    }
    return false;
}
