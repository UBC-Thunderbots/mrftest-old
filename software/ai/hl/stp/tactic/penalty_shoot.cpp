	...
	void PenaltyShoot::execute(caller_t& ca)
	{
		...
		if (world.enemy_team().size() > 0)
		{
			...
			else
			{
				...
				// out by pivoting.
				AI::HL::STP::Action::move(ca, world, player(),
					world.ball().position() - Point(Robot::MAX_RADIUS, Robot::MAX_RADIUS),
					(enemy_goal - world.ball().position() - Point(Robot::MAX_RADIUS, Robot::MAX_RADIUS)).orientation());

				AI::HL::STP::Action::pivot(ca, world, player(), world.ball().position(), Angle::of_radians(0.05));
				target = enemy_goal;
			}
		}
		else
		...
		// This used to be a call to a "Pivot Shoot" action, which seems to no longer exist?
