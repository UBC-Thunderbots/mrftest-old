...
namespace {
	...
	void BallPlacement::execute(caller_t& ca)
	{
		if ((world.ball().position() - world.ball_placement_position()).len() > 0.08) {
			if (!player().has_ball()) {
				Action::catch_ball(ca, world, player(), world.ball_placement_position());
			}
			else {
				Action::dribble(ca, world, player(), world.ball().position(), (world.ball_placement_position() - world.ball().position()).orientation());
			}
		}
		else if ((player().position() - world.ball().position()).len() < 0.5) {
			Action::move(ca, world, player(), world.ball().position() + (player().position() - world.ball().position()).norm() * 0.6);
		}
		else {
