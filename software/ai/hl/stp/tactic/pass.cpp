...
namespace {
	DegreeParam passer_tol_target(u8"when passer within this angle tol passee responds to passer direction (degrees)", u8"AI/HL/STP/Tactic/pass", 30.0, 0.0, 180.0);
	DoubleParam fast_velocity(u8"velocity of pass threshold", u8"AI/HL/STP/Tactic/pass", 1.0, 0.0, 1.0);
	DoubleParam negligible_velocity(u8"velocity to ignore", u8"AI/HL/STP/Tactic/pass", 0.05, 0.0, 1.0);
	DoubleParam passee_hack_dist(u8"Hack to get reciever to move more quickly to intercept pos by modifying dest (meters)", u8"AI/HL/STP/Tactic/pass", 0.0, 0.0, 1.0);
	DoubleParam passee_hack_appl(u8"dist from target when passee hack is applicable (meters)", u8"AI/HL/STP/Tactic/pass", 0.1, 0.001, 1.0);
	DoubleParam ball_region_param(u8" the radius (meters) in which passer must be with repect to ball before valid ", u8"AI/HL/STP/Tactic/pass", 1.0, 0.0, 5.0);
	BoolParam passer_depends_baller_can_shoot(u8" shot on net avaiable means that Passer should fail ", u8"AI/HL/STP/Tactic/pass", true);
	BoolParam passer_depends_calc_best_shot_target(u8" pass blocked means that Passer should fail ", u8"AI/HL/STP/Tactic/pass", true);
