...
namespace {
	 ...
	 */
	class Goalie2 final : public LegacyTactic {
		public:
			explicit Goalie2(World world, size_t defender_role) : LegacyTactic(world), defender_role(defender_role) {
			}
	 ...
	 * Goalie in a team of N robots.
	 */
	class Goalie final : public LegacyTactic {
		public:
			explicit Goalie(World world) : LegacyTactic(world) {
			}
	...
	class Defender final : public LegacyTactic {
		public:
			explicit Defender(World world, unsigned i, bool active_baller) : LegacyTactic(world), index(i), active_baller(active_baller) {
			}
	...
	void Goalie2::execute() {
		...
		// If enemy is in our defense area, go touch them so we get penalty kick
			if(AI::HL::Util::point_in_friendly_defense(world.field() , i.position())) {
				...
				Action::goalie_move(world, player, i.position());
				return;
			}
		}
		if (tdefend_goalie) {
			...
			Action::goalie_move(world, player, block_pos);
		} else {
			// solo
			AI::HL::STP::Action::lone_goalie(world, player);
		}
	...
	void Defender::execute() {
		...
		Action::defender_move(world, player, dest, active_baller);
	}
