	...
	void FreeKickToGoal::execute(caller_t& ca) {
		while (true) {
			...
			Angle total_angle = vertex_angle(gp1 , world.ball().position(), gp2).angle_mod().abs();
      		pair<Point, Angle> bestShotPair = Evaluation::get_best_shot_pair(world, player());

			//if the ball is really close to the far end of the field, run the corner-kick play, if not, run normal free kick play
			if ( world.ball().position().x > ( world.field().enemy_goal().x - world.field().defense_area_width() ) ) {
				Point targetDefensiveCrease = world.field().enemy_goal();
				targetDefensiveCrease.x = targetDefensiveCrease.x - world.field().defense_area_width() - 0.1; // set the target to be just on the stretch of the defense area
                // we want to chip the ball across the defensive area
		  ...
		  } else if (bestShotPair.second / total_angle > 0.15) { //player can see 15% of the net. Will shoot on goal
        		Action::shoot_target(ca, world, player(), bestShotPair.first);
			}
