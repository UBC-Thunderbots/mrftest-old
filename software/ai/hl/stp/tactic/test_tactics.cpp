...
{
   public:
    explicit MoveTestOrientation(World world, Point dest)
        : Tactic(world), dest(dest)
    {
    }
...
void MoveTestOrientation::execute(caller_t& ca)
{
    while (true)
    {
        AI::HL::STP::Action::pivot(
            ca, world, player(), Point(), Angle::zero(), 1);
        yield(ca);
    }

    //		// testing if rrt mvoes out of the field
    //		AI::Flags::MoveFlags flags = player().flags();
    //		if ((flags & AI::Flags::MoveFlags::CLIP_PLAY_AREA) !=
    ...
    ////			AI::HL::STP::Action::move_rrt(ca, world,
    /// player(),
    /// p);
    ////			yield(ca);
    ////		}
    ...
    //		AI::HL::STP::Action::move_slp(ca, world, player(),
    // original_pos);
}
