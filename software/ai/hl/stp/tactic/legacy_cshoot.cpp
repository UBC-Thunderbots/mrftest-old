...
namespace {
	class CShootGoal final : public LegacyTactic {
		public:
			explicit CShootGoal(World world, bool force) : LegacyTactic(world, true), kick_attempted(false), force(force),
	...
	void CShootGoal::execute() {
		...
		if (!player.has_ball() && timer < 15) {
			legacy_catch_ball(world, player, target);
			return;
		}
		...
		if (((std::get<0>(best_score) - player.position()).len() > 0.05) || 
			...
			dribble(world, player, std::get<0>(best_score), std::get<1>(best_score));
			return;
		}


		shoot_target(world, player, std::get<0>(best_score));
	}
