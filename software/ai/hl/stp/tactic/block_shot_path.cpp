...
{
   public:
    explicit BlockShotPath(World world, unsigned int index, double max_dist)
        : Tactic(world), index(index), max_dist_from_net(max_dist)
    {
    }
...
// Define Class player
// Compare the distance from the 'dest' and return the the player who has the
// shortest distance
...
void BlockShotPath::execute(caller_t &ca)
{
    while (true)
    {
        ...
        std::vector<std::pair<Player, const AI::HL::STP::Tactic::Tactic *>>
            tactic_assignments = get_tactic_assignment();
        ...
        // Enemy_threats ranks the enemy robots that are closer to the enemy
        // baller.
        std::vector<Robot> enemy_threats =
            AI::HL::STP::Evaluation::rank_enemy_passee_risk(
                world, obstacles, future_block_time);

        // Enemy_theats.size() is always 5.
        // If enemy_threats.size is smaller than index. all enemy_to_ is just
        // becomeing the point (0.0)
        if (enemy_threats.size() > index)
        {
            enemy_to_block =
                enemy_threats[index]
                    ? enemy_threats[index].position(future_block_time)
                    : Point();
        }
        ...
        std::pair<Point, Angle> risk_zone = angle_sweep_circles(
            enemy_to_block, goal_side, goal_opp, obstacles, Robot::MAX_RADIUS);
        // Angle ang = risk_zone.second * 1;

        Point destination =
            enemy_to_block + 0.35 * (risk_zone.first - enemy_to_block).norm();

        Point goal_intersect = line_intersect(
            enemy_to_block, risk_zone.first, goal_opp, goal_side);

        if ((enemy_to_block - goal_intersect).len() > max_dist_from_net)
        {
            destination =
                goal_intersect +
                max_dist_from_net * (enemy_to_block - goal_intersect).norm();

            // line_intersect(enemy_to_block, risk_zone.first,
            // Point(-world.field().width()*1.0/8.0,10.0),Point(-world.field().width()*1.0/8.0,
            // -10.0));
        }
        // Point destination = calc_block_cone(c1, c2, enemy_to_block,
        // Robot::MAX_RADIUS);
        if (destinations.size() > index)
        {
        ...
        Action::move(
            ca, world, player(), destination,
            (destination - player().position()).orientation());
...
Tactic::Ptr AI::HL::STP::Tactic::block_shot_path(
    World world, unsigned int index, double max_dist)
{
