...
{
   public:
    explicit BlockGoal(World world, Enemy::Ptr enemy)
        : Tactic(world), enemy(enemy)
    {
    }
...
Player BlockGoal::select(const std::set<Player>& players) const
{
    ...
    return *std::min_element(
        ...
        AI::HL::Util::CmpDist<Player>(enemy->evaluate().position()));
}
...
void BlockGoal::execute(caller_t& ca)
{
    while (true)
    {
        ...
        Point dirToGoal =
            (world.field().friendly_goal() - player().position().norm());
        Point target =
            player().position() + (AI::HL::STP::Action::block_threshold *
                                   Robot::MAX_RADIUS * dirToGoal);
