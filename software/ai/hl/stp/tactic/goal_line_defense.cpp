...
namespace
{
DoubleParam goal_line_defense_dist_from_goal_factor(
    u8"goal line defense dist from goal factor",
    u8"AI/HL/STP/Tactic/goal_line_defense", 1.15, 0.0, 10);
DoubleParam goal_line_defense_gap_between_defenders(
    u8"goal line defense gap between defenders",
    u8"AI/HL/STP/Tactic/goal_line_defense", AI::Common::Robot::MAX_RADIUS, 0.0,
    5);

class GoalLineDefense final : public Tactic
{
   public:
    explicit GoalLineDefense(World world, std::string position)
        : Tactic(world),
          position(position),
...
Player GoalLineDefense::select(const std::set<Player>& players) const
{
    return *std::min_element(
        ...
        AI::HL::Util::CmpDist<Player>(getDefendPoint()));
}
...
Point GoalLineDefense::getDefendPoint(std::string role) const
{
    ...
    Point goalside_traj;
    const double goal_crease_radius = world.field().defense_area_radius() +
    ...
    Point goalside_defpoint_traj =
        -ball_goalside_traj.norm(dist_from_goal_factor * goal_crease_radius);
...
Point GoalLineDefense::getDefendPoint() const
{
    ...
    const Point defend_point_other =
        getDefendPoint(position == "top" ? "bottom" : "top");
...
void GoalLineDefense::execute(caller_t& ca)
{
    while (true)
    {
        AI::HL::STP::Action::move(
            ca, world, player(), getDefendPoint(),
