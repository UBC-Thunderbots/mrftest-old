...
{
   public:
    explicit CShootGoal(World world, bool force)
        : Tactic(world),
          kick_attempted(false),
          force(force),
          timer(0),
          best_score(std::make_tuple(Point(0, 0), Point(0, 0), 0))
    {
        // world.friendly_team().signal_robot_removing().connect(sigc::mem_fun(this,
        // &ShootGoal::on_player_removed));
    }
   ...
   private:
    ...
    // if(passer.is() && world.friendly_team().get(index) ==
    // Player::CPtr(passer)){
    // passer.reset();
    // }
    // }
...
void CShootGoal::execute(caller_t &ca)
{
    ...
    if ((world.ball().position() - player().position()).len() >
        Robot::MAX_RADIUS + 0.02)
    {
    ...
    // first: find the best location to shoot at in a circle centered at the
    // baller with a radius of 0.5m
    ...
    for (double x = initial_position.x - 0.50; x < initial_position.x + 0.50;
         x += 0.04)
    {
        for (double y = initial_position.y - 0.50;
             y < initial_position.y + 0.50; y += 0.04)
        {
            if ((Point(x, y) - initial_position).len() < 0.45)
            {
                std::pair<Point, Angle> sweep_score = angle_sweep_circles(
                    Point(x, y), enemy_goal_positive, enemy_goal_negative,
                    obstacles, Robot::MAX_RADIUS);
                double shoot_score = sweep_score.second.to_degrees();

                for (auto p : world.enemy_team())
                {
                    if ((p.position() - Point(x, y)).len() <
                        Robot::MAX_RADIUS * 2 + 0.03)
                        shoot_score = 0.0;
                }

                if (dist(
                        Point(x, y),
                        Seg(enemy_goal_negative, enemy_goal_positive)) < 0.4)
                    shoot_score *= 0.1;

                shoot_score -= (Point(x, y) - player().position()).len() * 8;
                scores.push_back(std::make_tuple(
                    Point(x, y), sweep_score.first, shoot_score));
            }
        }
    }
    ...
    target = angle_sweep_circles(
                 std::get<0>(best_score), enemy_goal_positive,
                 enemy_goal_negative, obstacles, Robot::MAX_RADIUS)
                 .first;
    ...
    if (((std::get<0>(best_score) - player().position()).len() > 0.05) ||
        (fabs((player().orientation() -
               (std::get<1>(best_score) - player().position()).orientation())
                  .to_degrees()) > 2.0))
    {
        Action::dribble(
            ca, world, player(), std::get<0>(best_score),
            std::get<1>(best_score));
        return;
    }
...
void CShootGoal::draw_overlay(Cairo::RefPtr<Cairo::Context> ctx) const
{
    ...
    {
        ctx->set_source_rgba(
            std::get<2>(i) / 15, std::get<2>(i) / 15, std::get<2>(i) / 15, 0.5);
    ...
    ctx->arc(
        std::get<0>(best_score).x, std::get<0>(best_score).y, 0.02, 0.0,
        2 * M_PI);
