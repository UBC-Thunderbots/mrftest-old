...
constexpr Point ready_positions[2] = {Point(-AVOIDANCE_DIST, -SEPARATION_DIST),
                                      Point(-AVOIDANCE_DIST, SEPARATION_DIST)};
}

BEGIN_DEC(KickoffFriendly)
INVARIANT(
    (playtype(world, PlayType::PREPARE_KICKOFF_FRIENDLY) ||
     playtype(world, PlayType::EXECUTE_KICKOFF_FRIENDLY)) &&
...
while (!playtype(world, PlayType::EXECUTE_KICKOFF_FRIENDLY))
    yield(caller);
