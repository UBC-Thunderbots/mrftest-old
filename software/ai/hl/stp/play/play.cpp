...
namespace {
	...
	BoolParam goalie_required(u8"Play can't execute without goalie", u8"AI/HL/STP/Goalie", false);
	...
	std::vector<std::pair<Player, const AI::HL::STP::Tactic::Tactic*>> tactic_assignment;
}
...
const std::array<AI::HL::STP::Tactic::Tactic::Ptr, AI::HL::STP::TEAM_MAX_SIZE>& Play::get_tactics() const {
	return tactics;
}

const std::array<AI::HL::W::Player, AI::HL::STP::TEAM_MAX_SIZE>& Play::get_assignment() const {
...
bool Play::role_assignment(std::vector<bool> players_enabled) {
	...
	for (std::size_t i = 0; i < TEAM_MAX_SIZE; ++i) {
		...
		if (curr_assignment[i]) {
			tactic_assignment.push_back(std::make_pair(curr_assignment[i], tactics[i].get()));
		}
	}
...
void Play::tick(const std::vector<bool>& players_enabled) {
	...
	for (std::size_t i = 1; i < TEAM_MAX_SIZE; ++i) {
		...
		AI::Flags::MoveFlags extra_flags = AI::Flags::calc_flags(world.playtype());
...
#pragma GCC diagnostic ignored "-Wzero-as-null-pointer-constant" // boost pls
	_coroutine = coroutine_t([this](caller_t& caller) { caller(); execute(caller); });
