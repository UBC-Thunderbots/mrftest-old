...
class Play : public NonCopyable
{
   public:
     ...
     */
    const std::array<AI::HL::STP::Tactic::Tactic::Ptr, TEAM_MAX_SIZE>&
    get_tactics() const;
...
class PlayFactory : public Registerable<PlayFactory>
{
   public:
    /**
     * \brief A condition that must hold ALL THE TIME, for this play to be
     * considered and run.
     *
     * This is the ideal place to put conditions about playtype and minimum team
     * size.
     */
    virtual bool invariant(World world) const = 0;

    /**
     * \brief For a play to be considered applicable() and invariant() must be
     * true.
     *
     * This is only used once; after a play runs, applicable() is no longer
     * called.
     */
    virtual bool applicable(World world) const = 0;

    /**
     * \brief Constructs a new instance of the Play corresponding to this
     * PlayFactory.
     */
     ...
     *
     * \param[in] name a human-readable name for this Play, which must be
     * encoded in UTF-8
     */
    ...
    /**
     * \brief Used to determine the playbook this play belongs to (use with
     * bitmask)
     * Playbook 1 is standard play
     * Playbook 2 is mixed team defense
     * Playbook 3 is mixed team offense
     * ...
     * If you want a particular play to be available in all playbooks, simply
     * set it
     * to the maximum bitmask value (currently set to 1023 to allow a total of
     * 10 playbooks.)
     */
    IntParam playbook;
