...
void PassInfo::updateCurrentPoints(
    std::vector<PassInfo::passDataStruct> newPoints)
{
    currentPoints_mutex.lock();
...
void PassInfo::updateWorldSnapshot(World world)
{
    ...
    // in case we want another point as the passer other than the one closest to
    // the ball
    if (tacticInfo.use_stored_point_as_passer)
    {
        ...
        Player alt_passer = *std::min_element(
            world.friendly_team().begin(), world.friendly_team().end(),
            AI::HL::Util::CmpDist<Player>(tacticInfo.alt_passer_point));
        Player goalie = *std::min_element(
            world.friendly_team().begin(), world.friendly_team().end(),
            AI::HL::Util::CmpDist<Player>(world.field().friendly_goal()));
    ...
    else
    {
        ...
        // Player goalie =  *std::min_element(world.friendly_team().begin(),
        // world.friendly_team().end(),
        // AI::HL::Util::CmpDist<Player>(world.field().friendly_goal()));
    ...
    new_snapshot.friendly_goal_boundary =
