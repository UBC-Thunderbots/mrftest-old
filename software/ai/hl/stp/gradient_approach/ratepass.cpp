		...
		namespace STP {
			namespace GradientApproach {
				double ratePass(PassInfo::worldSnapshot snapshot, Point target,  double time_delay, double ball_velocity){
					...
					pass_quality = pass_quality * (0.5 + 0.5*shoot_score ); // give some importance to shooting (but not all)
					...
					pass_quality = pass_quality/(1+std::exp(200 * (2.5 - ball_velocity))); // strict requirement that ball vel > 2.5
					pass_quality = pass_quality/(1+std::exp(200 * (-6 +  ball_velocity )));  // strict requirement that ball vel < 6
