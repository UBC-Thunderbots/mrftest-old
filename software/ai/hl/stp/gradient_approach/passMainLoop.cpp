...
void superLoop(PassInfo::worldSnapshot snapshot, std::atomic_bool* stop_token)
{
    ...
    while (!(*stop_token))
    {
        std::vector<PassInfo::passDataStruct> newStartingPos =
            newPositions(snapshot, 4);
        passPointsLog.insert(
            passPointsLog.end(), newStartingPos.begin(), newStartingPos.end());
        ...
        {
            std::sort(
                passPointsLog.begin(), passPointsLog.end(), comparePassQuality);
            passPointsLog.erase(
                passPointsLog.begin() + 40, passPointsLog.end());
        }

        PassInfo::Instance().updateCurrentPoints(
            bestPassPositions(snapshot, passPointsLog, 2));
...
void testLoop(PassInfo::worldSnapshot snapshot)
{
    ...
    {
        passPointsLog.back().insert(
            passPointsLog.back().end(), newStartingPos.begin(),
            newStartingPos.end());
    }

    passPointsLog.push_back(merge(passPointsLog.back()));
    std::vector<PassInfo::passDataStruct> steppedData =
        stepForward(snapshot, passPointsLog.back());
...
{
    // Generate a list of potential points, then randomly select the number
    // specified by 'quantity' to be returned.
    // Current implementation is fairly inefficient when 'quantity' is smaller
    // than the number of potential points.
    // An improvement would be to randomly pick indices then calculate only
    // their positions.
    ...
    mergedEnemyPositions.push_back(
        (snapshot.enemy_positions.at(0) - snapshot.passer_position));

    bool foundCloseCopy = false;
    for (unsigned int i = 1; i < snapshot.enemy_positions.size(); i++)
    {
        foundCloseCopy = false;
        Point each_enemyPosition =
            (snapshot.enemy_positions.at(i) - snapshot.passer_position);
        for (unsigned int j = 0; j < mergedEnemyPositions.size(); j++)
        {
            if ((each_enemyPosition.orientation().angle_diff(
                    mergedEnemyPositions.at(j).orientation())) <
                ENEMY_ORIENTATION_THRESHHOLD)
            {
                foundCloseCopy = true;
                break;
            }
    ...
    friendlyRadii.push_back(
        (snapshot.passee_positions.at(0) - snapshot.passer_position).len());

    for (unsigned int i = 1; i < snapshot.passee_positions.size(); i++)
    {
        foundCloseCopy = false;
        double potential_radius =
            (snapshot.passee_positions.at(i) - snapshot.passer_position).len();
        for (unsigned int j = 0; j < friendlyRadii.size(); j++)
        {
            if (fabs(friendlyRadii.at(j) - potential_radius) <
                FRIENDLY_RADIUS_THRESHHOLD)
            {
                foundCloseCopy = true;
                break;
            }
        ...
        {
            friendlyRadii.push_back(
                (snapshot.passee_positions.at(i) - snapshot.passer_position)
                    .len());
        }
    }
    ...
    for (unsigned int i = 0; i < snapshot.passee_positions.size(); i++)
    {
        ...
        //This is set to 0 since if we are passing straight on dont need to
        delay
        lDataStruct.params.push_back(0);
        lDataStruct.params.push_back(4);
        lDataStruct.alpha = 0.5;
        */
        startingPositions.push_back(PassInfo::passDataStruct(
            snapshot.passee_positions.at(i).x,
            snapshot.passee_positions.at(i).y, 0, 4, 0.5));
    }
    ...
    for (unsigned int i = 0; i < mergedEnemyPositions.size() - 1; i++)
    {
        for (unsigned int j = 0; j < friendlyRadii.size(); j++)
        {
            startingPositions.push_back(estimateParams(
                snapshot, i, mergedEnemyPositions, j, friendlyRadii));
        }
    }

    if (quantity < startingPositions.size())
    {
        // Randomly order the vector, then truncate the vector to the desired
        // number of points
    ...
    std::vector<double> friendlyRadii)
{
    ...
    Angle targetOrientation = (mergedEnemyPositions.at(i).orientation() +
                               mergedEnemyPositions.at(i + 1).orientation()) /
                              2;
    Point lDataStructTarget =
        Point::of_angle(targetOrientation) * friendlyRadii.at(j) +
        snapshot.passer_position;

    Angle closestEnemyOrientation =
        targetOrientation.angle_diff(mergedEnemyPositions.at(0).orientation())
            .abs();
    int closestEnemyIndex = 0;
    for (unsigned int k = 1; k < mergedEnemyPositions.size(); k++)
    {
        Angle lClosestEnemyOrientation =
            (targetOrientation.angle_diff(
                 mergedEnemyPositions.at(k).orientation()))
                .abs();
    ...
    double ball_vel = closestEnemyOrientation.to_radians() *
                      mergedEnemyPositions.at(closestEnemyIndex).len() / V_MAX;
    ...
    double t_delay = shortest_dist / V_MAX - friendlyRadii.at(j) / ball_vel;
    if (t_delay < 0.5)
    {
        t_delay = 0.5;
    }
    ...
    lDataStruct.alpha = 0.5;
    */
    return PassInfo::passDataStruct(
        lDataStructTarget.x, lDataStructTarget.y, t_delay, ball_vel, 0.5);
}

PassInfo::passDataStruct stepForward(
    PassInfo::worldSnapshot snapshot, PassInfo::passDataStruct dataStruct)
{
    Point start_target =
        Point(dataStruct.params.at(0), dataStruct.params.at(1));
    int max_func_evals         = 40;
    std::vector<double> output = optimizePass(
        snapshot, start_target, dataStruct.params.at(2),
        dataStruct.params.at(3), max_func_evals);
    // PassInfo::passDataStruct lDataStruct;
    /*
    ...
    */
    ...
    // std::cout << "dX:" <<  dataStruct.params.at(0) - output.at(1) << " dY:"
    // << dataStruct.params.at(1) - output.at(2) << std::endl;
    // std::cout << "changeQuality" << output.at(0) - dataStruct.quality <<
    // std::endl;
    // std::cout << "alpha" << output.at(5) << " gradX: " << output.at(6) << "
    // gradY: " << output.at(7);
    return PassInfo::passDataStruct(
        output.at(1), output.at(2), output.at(3), output.at(4), output.at(5),
        output.at(0), output.at(6), output.at(7), output.at(8), output.at(9));
}

std::vector<PassInfo::passDataStruct> stepForward(
    PassInfo::worldSnapshot snapshot,
    std::vector<PassInfo::passDataStruct> dataStructs)
{
...
{
    // Compare all potential pass objects. For any two passes that are found
    // with a similar destination, the lower quality of the two is pruned.

    const double MERGE_THRESHHOLD = 0.2;
    double x1;
    double y1;
    double xdist;
    double ydist;
    ...
    {
        // check if current pass object is contained in remove-vector (if it is
        // it should be ignored)

        if (std::find(removed_objects.begin(), removed_objects.end(), i) ==
            removed_objects.end())
        {
            ...
            {
                if (std::find(
                        removed_objects.begin(), removed_objects.end(), n) ==
                    removed_objects.end())
                {
                    ...
                    {
                        if (potential_passes.at(i).quality >
                            potential_passes.at(n).quality)
                        {
    ...
    unsigned int numberPositions)
{
    ...
    std::sort(
        potential_passes.begin(), potential_passes.end(), comparePassQuality);
    ...
    for (std::size_t ii = 1; ii < potential_passes.size(); ii++)
    {
        ...
        Point relativeTarget =
            potential_passes.at(ii).getTarget() - snapshot.passer_position;
        for (std::size_t jj = 0; jj < bestPositions.size(); jj++)
        {
            Point relativeBestPos =
                bestPositions.at(jj).getTarget() - snapshot.passer_position;

            // If its closer to passer and the fastest distance
            if (((relativeTarget.len() < relativeBestPos.len()) &&
                 ((relativeTarget.orientation().angle_diff(
                       relativeBestPos.orientation()))
                          .to_radians() *
                      relativeTarget.len() <
                  0.2)))
            {
