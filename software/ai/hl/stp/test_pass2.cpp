...
namespace {
	...
	class TestPass2Factory : public HighLevelFactory {
		public:
			TestPass2Factory() : HighLevelFactory("Test STP Pass2") {
			}

			HighLevel::Ptr create_high_level(World &world) const;
	};

	TestPass2Factory factory_instance;

	class TestPass2 : public HighLevel {
		public:
			TestPass2(World &world) : world(world), targets(default_targets, default_targets + default_targets_n) {
				kicked_count = 0;
				kicked = false;
			}
		...
		private:
			...
			TestPass2Factory &factory() const {
...
Action::move(players[0], players[0]->orientation(), players[0]->position());
	...
	HighLevel::Ptr TestPass2Factory::create_high_level(World &world) const {
		HighLevel::Ptr p(new TestPass2(world));
		return p;
	}
}
