// Takes enemy positions and velocities, passee, passer, pass destination, kick
// speed, and delay time as input
// Evaluates the risk due to pass interception and proximity of enemy robots to
// destination position
// Returns a value between 0 and 1: where a higher number represents greater
// risk
...
double AI::HL::STP::Evaluation::getRatePassEnemyRisk(
    PassInfo::worldSnapshot snapshot, Point destination, double delay_time,
    double kickSpeed)
{
    double quality = 1;  // start assuming there is no risk

    // increase the risk at a given position based on the distance from all
    // enemy robots
    for (Point each_enemy_position : snapshot.enemy_positions)
    {
        ...
        quality =
            quality *
            (1 - ENEMY_PROXIMITY_IMPORTANCE * std::exp(-distance * distance));
    }

    // include the danger of the pass being intercepted
    quality = quality * (1 - dangerInterception(
                                 snapshot, destination, delay_time, kickSpeed));
...
double AI::HL::STP::Evaluation::dangerInterception(
    PassInfo::worldSnapshot snapshot, Point destination, double delay_time,
    double kickSpeed)
{
    ...
    // Estimate of the time it takes the enemy to respond to our actions, should
    // be observed
    ...
    // Enemies are assumed to move at constant velocity then stop in their
    // position
    // at 'future_time'
    if (delay_time > 0.45)
    {
    ...
    {
        future_time = delay_time;
    }

    // This has 1 more element than enemy_team because a worst case prediction
    // for the closest enemy robot is added.
    // The closest enemy robot is assumed to move as fast as possible (without
    // crashing) towards the passer
    ...
    {
        projected_enemy_positions.at(i) =
            snapshot.enemy_positions.at(i) +
            snapshot.enemy_velocities.at(i) * future_time;
        current_distance =
            (snapshot.passer_position - projected_enemy_positions.at(i)).len();
    ...
    // Finds how far the closest enemy could travel (calculated based on zero
    // initial velocty)
    // The initial velocity is then multiplied by future_time and added to the
    // result to try to
    // account for the zero initial velocity assumption
    if (delay_time > 2 * ENEMY_V_MAX / ENEMY_A_MAX)
    {
        // distance that can be travelled by accelerating to max speed, going at
        // max speed, then
        // decelerating to zero velocity
        // oversimplified- they will actually start slowing down at the end not
        // the middle
        distance_travelled =
            ENEMY_V_MAX * ENEMY_V_MAX / ENEMY_A_MAX +
            (delay_time - 2 * ENEMY_V_MAX / ENEMY_A_MAX) * ENEMY_V_MAX;
    }
    else
    {
    ...
    // Additional virtual enemy to be avoided- works on assumption that closest
    // enemy to ball moves towards ball
    if (shortest_distance > distance_travelled)
    {
        projected_enemy_positions.at(num_enemies) =
            projected_closest_enemy_position *
                (1 - distance_travelled / shortest_distance) +
            passer_pos * (distance_travelled / shortest_distance);
    }
    else
    {
    ...
    for (unsigned int i = 0; i <= num_enemies; i++)
    {
        ...
        l2 = l2 * l2;
        r  = ((projected_enemy_positions.at(i) - passer_pos)
                 .dot(destination - passer_pos)) /
            l2;
        ...
        {
            dist_intercept =
                (projected_enemy_positions.at(i) - passer_pos).len();
            t_intercept = delay_time;
        }
        else if (r > 1)
        {
            dist_intercept =
                (projected_enemy_positions.at(i) - destination).len();
        ...
        else
        {
            ...
            dist_intercept =
                (projected_enemy_positions.at(i) - projected_vec).len();
        ...
        if (t_move > 2 * ENEMY_V_MAX / ENEMY_A_MAX)
        {
            q = dist_intercept - R_RADIUS - DIST_UNCERTAINTY -
                ENEMY_V_MAX * ENEMY_V_MAX / ENEMY_A_MAX -
                (t_move - 2 * ENEMY_V_MAX / ENEMY_A_MAX) * ENEMY_V_MAX;
        }
        else if (t_move > 0)
        {
            q = dist_intercept - R_RADIUS - DIST_UNCERTAINTY -
                ENEMY_A_MAX / 4 * t_move * t_move;
        }
        else
        {
            q = dist_intercept - R_RADIUS - DIST_UNCERTAINTY;
        ...
        danger = 1 / (1 + std::exp(
                              W_q * q / ((TIME_UNCERTAINTY * delay_time + 1) *
                                         (t_intercept - delay_time))));
