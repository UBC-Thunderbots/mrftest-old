...
double AI::HL::STP::Evaluation::enemy_passee_risk(World world, std::vector<Point> obstacles, Robot enemy_passee, Robot enemy_baller, double future_block_time){
  
		bool goalie_top = true;
		std::pair<Point, Point> friendly_goal_posts = world.field().friendly_goal_boundary();
		...
		std::pair<Point, Angle> risk_zone = angle_sweep_circles(enemy_passee.position(future_block_time), goal_opp, goal_side, obstacles , Robot::MAX_RADIUS);
...
std::vector<Robot> AI::HL::STP::Evaluation::rank_enemy_passee_risk(World world, std::vector<Point> obstacles, double future_block_time) {
	...
	for (std::size_t i = 0; i < enemy_team.size(); ++i){
	  if(enemy_team[i].pattern() != enemy_baller.pattern()){
	    ...
	    double risk = enemy_passee_risk(world, obstacles, enemy_team[i], enemy_baller, future_block_time);
