...
namespace
{
BoolParam calc_baller_always_return(
    u8"Calc baller always return", u8"AI/HL/STP/ball", true);

BoolParam smart_possess_ball(
    u8"Smart possess ball (instead of has ball only)", u8"AI/HL/STP/ball",
    true);

DoubleParam enemy_pivot_threshold(
    u8"circle radius in front of enemy robot to consider possession (meters)",
    u8"AI/HL/STP/ball", 0.1, 0.0, 1.0);
...
void update_enemies_by_grab_ball_dist(World world)
{
    ...
    for (Robot i : enemies)
    {
        ...
        Evaluation::calc_fastest_grab_ball_dest(
            world.ball().position(), world.ball().velocity(), i.position(),
            dest);
...
DoubleParam Evaluation::pivot_threshold(
    u8"circle radius in front of robot to enable pivot (meters)",
    u8"AI/HL/STP/ball", 0.1, 0.0, 1.0);
...
bool Evaluation::behind_ball_within_dist(
    World world, Player player, Point target, double dist, Angle angle_thresh)
{
    if (player.has_ball())
    {
        return true;
    ...
    if (diff.len() < (dist + Robot::MAX_RADIUS) &&
        angle_diff.abs() < angle_thresh)
    {
        return true;
    }
...
Point Evaluation::calc_fastest_grab_ball_dest(World world, Player player)
{
    Point dest;
    Evaluation::calc_fastest_grab_ball_dest(
        world.ball().position(), world.ball().velocity(), player.position(),
        dest);
    return dest;
}
...
#warning TODO this seems to return values way to small to actually get robots in front of a moving ball. It seems to still approximate a stopped one
    return world.ball().position() +
           0.03 * world.ball().velocity() *
               (world.ball().position() - robot.position()).len();
}
