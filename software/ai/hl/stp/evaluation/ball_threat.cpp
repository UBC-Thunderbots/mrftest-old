...
namespace
{
DoubleParam steal_threshold(
    u8"Steal threshold: distance of ball from enemy (robot radius)",
    u8"AI/HL/STP/evaluation", 1.1, 0.8, 5.0);

DoubleParam negligible_velocity(
    u8"speed goalie should ignore direction of ball", u8"AI/HL/STP/evaluation",
    0.05, 1e-4, 1.0);
}
...
BallThreat AI::HL::STP::Evaluation::evaluate_ball_threat(World world)
{
    ...
    {
        std::sort(
            ball_threat.enemies.begin(), ball_threat.enemies.end(),
            AI::HL::Util::CmpDist<Robot>(world.ball().position()));

        ball_threat.threat = ball_threat.enemies[0];

        ball_threat.threat_distance =
            (ball_threat.threat.position() - world.ball().position()).len();
        ...
        for (std::size_t i = 0; i < ball_threat.enemies.size(); ++i)
        {
            ...
            if (unique_line_intersect(a, b, c, d))
            {
                ...
                ball_threat.activate_steal =
                    !(vel.len() > negligible_velocity &&
                      inter.y <= std::max(a.y, b.y) &&
                      inter.y >= std::min(a.y, b.y));
            }
        }

        // steal mechanism activation when ball is close enough
        if ((ball_threat.threat.position() - world.ball().position()).len() *
                Robot::MAX_RADIUS <
            steal_threshold)
        {
...
{
    if (world.ball().velocity().lensq() < negligible_velocity ||
...
Point AI::HL::STP::Evaluation::goalie_shot_block(
    AI::HL::W::World world, const AI::HL::W::Player player)
{
