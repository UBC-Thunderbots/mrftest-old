...
calc_best_shot_all(AI::HL::W::World world, AI::HL::W::Player player, double
radius = 1.0);
std::vector<std::pair<Point, double>> openings =
AI::HL::Util::calc_best_shot_all(world, player);
...
for(std::vector<std::pair<Point, double>>::iterator it = openings.begin(); it!=
openings.end(); it++){
...
if( angle_diff(ang_1, centre_ang) > both || angle_diff(ang_2, centre_ang) >
both){
continue;
}
if(it->second > best.second){
