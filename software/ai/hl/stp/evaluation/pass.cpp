...
namespace {
	...
	double def_passee_scoring_function(const World &world, const std::vector<Point> &enemy_pos, const Point &dest, const std::vector<Point> &dont_block) {
		// can't be too close to enemy
		double closest_enemy = world.field().width();
		for (std::size_t i = 0; i < enemy_pos.size(); ++i) {
			double dist = (enemy_pos[i] - dest).len();
	...
	bool calc_def_passee_position_best(const World &world, const std::vector<Point> &enemy_pos, const std::vector<Point> &dont_block, Point &best_pos) {
		// divide up into grids
		const double x1 = -world.field().length() / 2, x2 = -x1;
		const double y1 = -world.field().width() / 2, y2 = -y1;
		...
		for (int i = 0; i < grid_x; ++i) {
			for (int j = 0; j < grid_y; ++j) {
				...
				// TEMPORARY HACK!!
				// ensures that we do not get too close to the enemy defense area.
				// and too near our own goal
				const double their_goal_dist = (pos - world.field().enemy_goal()).len();
				const double our_goal_dist = (pos - world.field().enemy_goal()).len();
				if (their_goal_dist < world.field().goal_width() || our_goal_dist < world.field().width() / 2) {
					continue;
				}

				const double score = def_passee_scoring_function(world, enemy_pos, pos, dont_block);
				if (score < -1e50) {
					continue;
				}
				if (score > best_score) {
