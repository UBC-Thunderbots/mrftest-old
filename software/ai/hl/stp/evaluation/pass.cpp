...
bool Evaluation::passee_facing_ball(const World& world, Player::CPtr passee) {
	return player_within_angle_thresh(passee, world.ball().position(), receive_angle_threshold);
}

bool Evaluation::passee_facing_passer(Player::CPtr passer, Player::CPtr passee) {
	return player_within_angle_thresh(passee, passer->position(), receive_angle_threshold);
}
