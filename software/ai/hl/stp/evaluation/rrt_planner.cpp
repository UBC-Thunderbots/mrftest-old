...
namespace
{
IntParam iteration_limit(
    u8"Number of iterations to go through before we give best partial path",
    u8"AI/Nav/RRT", 200, 10, 2000);
DoubleParam threshold(
    u8"Distance to destination when we stop looking for a path (m)",
    u8"AI/Nav/RRT", 0.08, 0, 1.0);
DoubleParam step_distance(
    u8"Distance to extend the tree on each step (m)", u8"AI/Nav/RRT", 0.1, 0,
    1.0);
...
Point Evaluation::RRT::random_point(World world)
{
    ...
    double random_x =
        ((std::rand() % static_cast<int>(world.field().length() * 100)) -
         (world.field().length() * 50)) /
        100;
    double random_y =
        ((std::rand() % static_cast<int>(world.field().width() * 100)) -
         (world.field().width() * 50)) /
        100;
...
Point Evaluation::RRT::choose_target(
    World world, Point goal, Player player, std::vector<Point> way_points)
{
    ...
    {
        std::size_t i =
            static_cast<std::size_t>(std::rand()) % way_points.size();
        return way_points[i];
    }
...
Glib::NodeTree<Point> *Evaluation::RRT::nearest(
    Glib::NodeTree<Point> *rrt_tree, Point target)
{
    ...
    // iterate through all the nodes in the tree, finding which is closest to
    // the target
...
Point Evaluation::RRT::extend(
    World world, Player player, Glib::NodeTree<Point> *start, Point target,
    MoveFlags added_flags)
{
    Point extend_point =
        start->data() + ((target - start->data()).norm() * step_distance);
...
std::vector<Point> Evaluation::RRT::rrt_plan(
    World world, Player player, Point goal, std::vector<Point> way_points,
    bool post_process, MoveFlags added_flags)
{
    ...
    // should loop until distance between the last node added and goal is less
    // than threshold
    while (distance(last_added, goal) > threshold &&
    ...
    if (!found_path)
    {
        ...
        // set the last added as the node closest to the goal if we reach the
        // iteration limit
        // because the last added could be anything and we use it for tracing
        // back the path
    ...
    while (iterator != &rrt_tree)
    {
        ...
        if (found_path)
        {
            // std::dynamic_pointer_cast<Waypoints>(player.way_points[static_cast<std::size_t>(std::rand())
            // % player.NUM_WAYPOINTS] = iterator->data();
        }
    }
    ...
    // path post processing, try to go in a straight line until we hit an
    // obstacle
    ...
    // just use the current player position as the destination if we are within
    ...
    if (final_points.empty())
    {
        final_points.push_back(player.position());
                 ...
                 final_points[final_points.size() - 1], goal, world, player))
    {
