...
double Plan::get_goal_post_trespass(
    Point cur, Point dst, World world, Player player)
{
    double violate       = 0.0;
...
double Plan::get_friendly_trespass(
    Point cur, Point dst, World world, Player player)
{
    double violate = 0.0;
    // avoid enemy robots
...
double Plan::get_ball_stop_trespass(
    Point cur, Point dst, World world, Player player)
{
    double violate       = 0.0;
...
double Plan::get_defense_area_trespass(
    Point cur, Point dst, World world, Player player)
{
    const Field &f      = world.field();
...
double Plan::get_offense_area_trespass(
    Point cur, Point dst, World world, Player player)
{
    const Field &f      = world.field();
...
double Plan::get_ball_tiny_trespass(
    Point cur, Point dst, World world, Player player)
{
    const Ball &ball     = world.ball();
...
double Plan::get_ball_regular_trespass(
    Point cur, Point dst, World world, Player player)
{
    const Ball &ball     = world.ball();
...
double Plan::get_penalty_friendly_trespass(
    Point cur, Point dst, World world, Player player)
{
    const Ball &ball = world.ball();
    const Field &f   = world.field();
...
double Plan::get_penalty_enemy_trespass(
    Point cur, Point dst, World world, Player player)
{
    const Ball &ball = world.ball();
    const Field &f   = world.field();
...
struct Violation final
{
    ...
    explicit Violation(Point cur, Point dst, World world, Player player)
        : enemy(0.0),
          friendly(0.0),
          play_area(0.0),
          ball_stop(0.0),
    ...
    explicit Violation(
        Point cur, Point dst, World world, Player player, MoveFlags added_flags)
        : enemy(0.0),
          friendly(0.0),
          play_area(0.0),
          ball_stop(0.0),
    ...
    static Violation get_violation_amount(
        Point cur, Point dst, World world, Player player, MoveFlags extra_flags)
    {
...
void process_obstacle(
    std::vector<Point> &ans, World world, Player player, Point segA, Point segB,
    double dist, int num_points)
{
...
std::vector<Point> Plan::get_destination_alternatives(
    Point dst, World world, Player player)
{
...
bool Plan::isNewPathBetter(
    const std::vector<Point> &oldPath, const std::vector<Point> &newPath,
    const Point &target)
{
    double oldPathLength = 0;
    double newPathLength = 0;
    for (unsigned int i = 0; i < std::min(oldPath.size(), newPath.size()) - 1;
         i++)
    {
    ...
    bool better =
        newPathLength / oldPathLength < 0.8 &&
        (newPath.back() - target).len() < (oldPath.back() - target).len();
    LOGF_INFO(u8"new path better: %1", better);
    return better;
}

bool Plan::valid_path(
    Point cur, Point dst, World world, Player player, MoveFlags extra_flags)
{
...
std::vector<Point> Plan::get_obstacle_boundaries(
    World world, Player player, MoveFlags added_flags)
{
...
std::pair<Point, AI::Timestamp> Plan::get_ramball_location(
    Point dst, World world, Player player)
{
...
double Plan::estimate_action_duration(
...
/*
bool intercept_flag_handler(World world, Player player,
...
path_orientation));
                ...
                if (estimate_action_duration(path_points_with_angle) <
