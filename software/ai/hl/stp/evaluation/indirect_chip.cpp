...
namespace {
	...
	DoubleParam MIN_CHIP_TRI_EDGE_LEN(u8"the min edge len of the chip target tri required to be valid", u8"AI/HL/STP/Tactic/indirect_chip", 0.8, 0.0, 100.0);
	DoubleParam MIN_CHIP_TRI_EDGE_ANGLE(u8"the min angle (deg) between chip triangles edges required to be valid", u8"AI/HL/STP/Tactic/indirect_chip", 20, 0.0, 180);
	DoubleParam CHIP_CHERRY_POWER_DOWNSCALE(u8"percentage of distance to center of triangle to return as target", u8"AI/HL/STP/Tactic/indirect_chip", 0.85, 0.0, 100.0);
}


std::pair<Point, Angle> AI::HL::STP::Evaluation::indirect_chip_target(World world, Player player) {
	...
	// The triangle formed by the enemy goalposts and the ball. Any robots in this triangle could block a chip/shot
	Triangle chip_target_area = triangle(world.ball().position(), enemy_goal_positive, enemy_goal_negative);
	double rough_chip_dist = (world.field().enemy_goal() - world.ball().position()).len() * (CHIP_TARGET_FRACTION * 0.9);

	std::vector<Vector2> blocking_robots;

	// Adds any enemies that are in the block triangle and further than the chip dist to the blocking_robots vector
	for (auto i : world.enemy_team()) {
		if((contains(chip_target_area, i.position()) || offset_to_line(world.ball().position(), enemy_goal_negative, i.position()) <= Robot::MAX_RADIUS
				...
				&& ((i.position() - world.ball().position()).len() > rough_chip_dist + Robot::MAX_RADIUS)) {
			blocking_robots.push_back(i.position());
		}
	}

	// Adds any friendly robots that are in the block triangle and further than the chip dist to the blocking_robots vector
	...
	Point chip_net_target = angle_sweep_circles(world.ball().position(), enemy_goal_positive, enemy_goal_negative, blocking_robots, Robot::MAX_RADIUS).first;
	Angle chip_angle = angle_sweep_circles(world.ball().position(), enemy_goal_positive, enemy_goal_negative, blocking_robots, Robot::MAX_RADIUS).second;
	...
	Point max_target = world.ball().position() + chip_dir.norm(total_chip_dist * CHIP_POWER_BOUNCE_THRESHOLD);
	Point target;

	// Chipper is in our half of the field. Set target to center of net so no carpeting
	if(player.position().x < 0.0) {
		target = world.ball().position() + (world.field().enemy_goal() - world.ball().position()).norm(total_chip_dist * CHIP_TARGET_FRACTION);

		//target should never be further than max_target
		if((target - world.ball().position()).len() - (max_target - world.ball().position()).len() > 0.0)
			target = world.ball().position() + (world.field().enemy_goal() - world.ball().position()).norm(total_chip_dist * CHIP_POWER_BOUNCE_THRESHOLD);

		//target should never be furhter than MAX_CHIP_POWER
		if((target - world.ball().position()).len() > MAX_CHIP_POWER)
			target = world.ball().position() + (world.field().enemy_goal() - world.ball().position()).norm(MAX_CHIP_POWER);
	}
	else {// Try to chip for a (indirect) goal
		target = world.ball().position() + (chip_dir.norm(total_chip_dist * CHIP_TARGET_FRACTION));
		
		//target should never be further than max_target
		if((target - world.ball().position()).len() - (max_target - world.ball().position()).len() > 0.0)
			target = world.ball().position() + chip_dir.norm(total_chip_dist * CHIP_POWER_BOUNCE_THRESHOLD);
	...
	//LOGF_INFO(u8"Chipper can see: %1/1 of net, CHIP_DIST: %2", chip_angle, (target - world.ball().position()).len());
...
Point AI::HL::STP::Evaluation::deflect_off_enemy_target(World world) {
	...
	double closestEdgeY;
	//Finds the y value of the closest edge of the field (likely where the ball is)
	if(world.ball().position().y <= 0.0)
		closestEdgeY = world.field().friendly_corner_neg().y;
	else
		closestEdgeY = world.field().friendly_corner_pos().y;

	//Find the enemy that's blocking a shot that's closest to the edge of the field
	for (auto i : world.enemy_team()) {
		if((contains(chip_target_area, i.position()) || offset_to_line(world.ball().position(), enemy_goal_negative, i.position()) <= Robot::MAX_RADIUS
				|| offset_to_line(world.ball().position(), enemy_goal_positive, i.position()) <= Robot::MAX_RADIUS)
	...
	//want to shoot at the edge of a robot so the ball deflects towards the edge of the field
	...
	if(abs((world.ball().position() + dir + dirPerp).y - closestEdgeY) > abs((world.ball().position() + dir - dirPerp).y - closestEdgeY))
...
std::pair<Point, bool> AI::HL::STP::Evaluation::indirect_chipandchase_target(World world) {
	...
	//LOGF_INFO(u8"NUMBER OF NON_GOALIE_ENEMIES: %1", non_goalie_enemy_positions.size());
	std::vector<Triangle> allTriangles = AI::HL::STP::Evaluation::get_all_triangles(world, non_goalie_enemy_positions);
	//LOGF_INFO(u8"NUMBER OF ALLTRIANGLES: %1", allTriangles.size());
	std::vector<Triangle> target_triangles = AI::HL::STP::Evaluation::filter_open_triangles(allTriangles, all_enemy_positions);
	//LOGF_INFO(u8"NUMBER OF TARGETTRIANGLES AFTER FILTEROPEN: %1", target_triangles.size());
	target_triangles = AI::HL::STP::Evaluation::remove_outofbounds_triangles(world, target_triangles);
	//LOGF_INFO(u8"NUMBER OF TRIANGLES AFTER REMOVING OUTOFBOUNDS: %1", target_triangles.size());

	if(!target_triangles.empty()) {
		std::pair<Triangle, bool> largest_triangle = AI::HL::STP::Evaluation::get_largest_triangle(target_triangles, MIN_CHIP_TRI_AREA, MIN_CHIP_TRI_EDGE_LEN);
		Triangle t = largest_triangle.first;
		bool valid = largest_triangle.second;
		Point target = AI::HL::STP::Evaluation::get_triangle_center_and_area(t).first;
		target = target.norm((target - world.ball().position()).len() * CHIP_CHERRY_POWER_DOWNSCALE);

		//target should never be furhter than MAX_CHIP_POWER
		if((target - world.ball().position()).len() > MAX_CHIP_POWER)
			target = world.ball().position() + (target - world.ball().position()).norm(MAX_CHIP_POWER);
...
std::vector<Triangle> AI::HL::STP::Evaluation::get_all_triangles(World world, std::vector<Point> enemy_players) {
	std::vector<Triangle> triangles;
	//std::vector<Point> corners = AI::HL::STP::Evaluation::get_chip_target_area_corners(world, CHIP_TARGET_AREA_INSET);
...
std::pair<Triangle, bool> AI::HL::STP::Evaluation::get_largest_triangle(std::vector<Triangle> allTriangles, double min_area, double min_edge_len, double min_edge_angle) {
	Triangle largest = allTriangles[0];
	double largest_area = AI::HL::STP::Evaluation::get_triangle_center_and_area(largest).second;

	bool valid = false;

	for(unsigned int i = 0; i < allTriangles.size(); i++) {
		Triangle t = allTriangles[i];
		double area = AI::HL::STP::Evaluation::get_triangle_center_and_area(t).second;
		...
		if(area > largest_area && area >= min_area
				&& l1 >= min_edge_len && l2 >= min_edge_len && l3 >= min_edge_len
				&& a1.to_degrees() >= min_edge_angle && a2.to_degrees() >= min_edge_angle && a3.to_degrees() >= min_edge_angle) {
			largest = t;
			largest_area = area;
			valid = true;
		}
...
std::vector<Triangle> AI::HL::STP::Evaluation::filter_open_triangles(std::vector<Triangle> triangles, std::vector<Point> enemy_players) {
	std::vector<Triangle> filtered_triangles;
	bool containsEnemy = false;
	for(unsigned int i = 0; i < triangles.size(); i++) {
		//create a triangle that is slightly smaller so the robots making up the vertices
		//are not counted as in the triangle
		Point p1 = triangles[i][0] + ((AI::HL::STP::Evaluation::get_triangle_center_and_area(triangles[i]).first) - triangles[i][0]).norm(2.5 * Robot::MAX_RADIUS);
		Point p2 = triangles[i][1] + ((AI::HL::STP::Evaluation::get_triangle_center_and_area(triangles[i]).first) - triangles[i][1]).norm(2.5 * Robot::MAX_RADIUS);
		Point p3 = triangles[i][2] + ((AI::HL::STP::Evaluation::get_triangle_center_and_area(triangles[i]).first) - triangles[i][2]).norm(2.5 * Robot::MAX_RADIUS);
		Triangle t = triangle(p1, p2, p3);
		containsEnemy = false;
...
std::vector<Triangle> AI::HL::STP::Evaluation::remove_outofbounds_triangles(World world, std::vector<Triangle> triangles) {
	std::vector<Triangle> valid_triangles;
	std::vector<Point> chip_area_corners = AI::HL::STP::Evaluation::get_chip_target_area_corners(world, CHIP_TARGET_AREA_INSET);
	Point center;
	...
	for(unsigned int i = 0; i < triangles.size(); i++) {
		...
		center = AI::HL::STP::Evaluation::get_triangle_center_and_area(triangles[i]).first;
...
std::pair<Point, double> AI::HL::STP::Evaluation::get_triangle_center_and_area(Triangle triangle) {
	Point p1 = triangle[0];
	Point p2 = triangle[1];
	Point p3 = triangle[2];
	...
	double area = abs(0.5 * ((p2.x-p1.x)*(p3.y-p1.y) - (p3.x-p1.x)*(p2.y-p1.y)));
...
std::vector<Point> AI::HL::STP::Evaluation::get_chip_target_area_corners(World world, double inset) {
