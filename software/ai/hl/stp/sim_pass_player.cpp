...
namespace {
	...
	class SimPassPlayer : public HighLevel {
		public:
			SimPassPlayer(World &world) : world(world), orient(0.0), adj_orientation(0.0, 0.0, 2 * M_PI, 0.1 * M_PI, 0.5 * M_PI), hsb_orientation(adj_orientation), xpos(0.0), adj_xposition(0.0, 0.0, 2 * M_PI, 0.1 * M_PI, 0.5 * M_PI), hsb_xposition(adj_xposition), ypos(0.0), adj_yposition(0.0, 0.0, 2 * M_PI, 0.1 * M_PI, 0.5 * M_PI), hsb_yposition(adj_yposition) {
		...
		private:
			...
			void tick() {
				...
				if (friendly.size() == 0) {
					...
					double theta = -std::atan(ball_vel.y / ball_vel.x);
					Point right_intersect = Point(receiver_pos.rotate(theta).x, ball_pos.rotate(theta).y).rotate(-theta);
					receiver->move(right_intersect, M_PI - ball_vel.orientation(), Point());
					to_draw = right_intersect;
					to_draw_orient = M_PI - ball_vel.orientation();
				}
			}
			Gtk::VBox ui_box;
			...
			void draw_overlay(Cairo::RefPtr<Cairo::Context> ctx) {
				...
				ctx->arc(to_draw.x, to_draw.y, 0.09, to_draw_orient - 0.2 * M_PI, to_draw_orient + 0.2 * M_PI);
			}
	};
