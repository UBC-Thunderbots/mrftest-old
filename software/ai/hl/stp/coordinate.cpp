...
Coordinate::Coordinate()
    : world(), y_type(YType::ABSOLUTE), o_type(OriginType::ABSOLUTE)
{
}

Coordinate::Coordinate(Point pos)
    : world(), y_type(YType::ABSOLUTE), o_type(OriginType::ABSOLUTE), pos(pos)
{
}

Coordinate::Coordinate(const Coordinate &coord)
    : world(coord.world.get() ? new World(*coord.world.get()) : nullptr),
      y_type(coord.y_type),
      o_type(coord.o_type),
      pos(coord.pos)
{
...
Coordinate::Coordinate(World world, Point pos, YType y_type, OriginType o_type)
    : world(new World(world)), y_type(y_type), o_type(o_type), pos(pos)
{
}
...
std::size_t Coordinate::hash() const
{
    std::size_t acc = 5;
    acc =
        acc * 17 + std::hash<unsigned int>()(static_cast<unsigned int>(y_type));
    acc =
        acc * 17 + std::hash<unsigned int>()(static_cast<unsigned int>(o_type));
