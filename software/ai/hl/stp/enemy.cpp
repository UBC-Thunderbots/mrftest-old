...
namespace
{
BoolParam use_grab_ball_pos(
    u8"enemy closest use grab ball position", u8"AI/HL/STP/enemy", true);
...
class ClosestFriendlyGoal final : public Enemy
{
   ...
   private:
    ...
    Robot evaluate() const override
    {
        ...
        // TODO: cache this
        std::nth_element(
            enemies.begin(), enemies.begin() + index, enemies.end(),
            AI::HL::Util::CmpDist<Robot>(world.field().friendly_goal()));
...
{
   public:
    explicit ClosestRobot(World w, unsigned int robot, unsigned int i)
        : world(w), robot(robot), index(i)
    {
    }
   ...
   private:
    ...
    Robot evaluate() const override
    {
        // Remember that the closest robot to robot of index i is that robot
        // itself.
        if (world.enemy_team().size() > index)
        {
            auto robots = AI::HL::Util::get_robots(world.enemy_team());
            std::nth_element(
                robots.begin(), robots.begin() + index, robots.end(),
                AI::HL::Util::CmpDist<Robot>(
                    world.enemy_team()[robot].position()));
            return robots[index];
        }
        return Robot();
    }
...
{
   public:
    explicit ClosestFriendlyPlayer(World w, Player player, unsigned int i)
        : world(w), player(player), index(i)
    {
    }
   ...
   private:
    ...
    Robot evaluate() const override
    {
        ...
        std::nth_element(
            enemies.begin(), enemies.begin() + index, enemies.end(),
            AI::HL::Util::CmpDist<Robot>(player.position()));
...
Enemy::Ptr AI::HL::STP::Enemy::closest_friendly_goal(
    World world, unsigned int i)
{
...
Enemy::Ptr AI::HL::STP::Enemy::closest_robot(
    World world, unsigned int robot, unsigned int i)
{
    return std::make_shared<ClosestRobot>(world, robot, i);
}

Enemy::Ptr AI::HL::STP::Enemy::closest_friendly_player(
    World world, Player player, unsigned int i)
{
