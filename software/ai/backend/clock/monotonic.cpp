...
Monotonic::Monotonic()
    : tfd(create_timerfd(CLOCK_MONOTONIC)),
      overflow_message(
          u8"Timer overflow!", Annunciator::Message::TriggerMode::EDGE,
          Annunciator::Message::Severity::LOW)
{
    const uint64_t nanoseconds =
        (UINT64_C(1000000000) + TIMESTEPS_PER_SECOND / 2) /
        TIMESTEPS_PER_SECOND;
    tfd.set_blocking(false);
    itimerspec tspec;
    tspec.it_interval.tv_sec =
        static_cast<time_t>(nanoseconds / UINT64_C(1000000000));
    tspec.it_interval.tv_nsec =
        static_cast<long>(nanoseconds % UINT64_C(1000000000));
    ...
    Glib::signal_io().connect(
        sigc::mem_fun(this, &Monotonic::on_readable), tfd.fd(), Glib::IO_IN);
}
