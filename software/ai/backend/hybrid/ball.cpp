...
namespace {
	DoubleParam BALL_DECAY_CONSTANT("Ball Decay Constant", "Backend/Hybrid", 99.0, 0.0, 100.0);
}
...
void Ball::update(Point pos, timespec ts) {
	...
	pred.add_measurement(neg ? -pos : pos, timespec_sub(ts, double_to_timespec(HYBRID_LOOP_DELAY)));
}
