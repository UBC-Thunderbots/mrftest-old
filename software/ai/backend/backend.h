 ...
 *
 * A backend must get the state of the world, expose it to the AI, accept
 * commands from the AI, and deliver those commands into the world.
 */
class Backend : public Visualizable::World, public NonCopyable
{
   public:
    /**
     * \brief The possible values indicating which end of the field the team is
     * defending.
     */
    enum class FieldEnd
    {
    ...
    /**
     * \brief Returns the number of table rows the backend's main tab UI
     * controls will consume.
     *
     ...
     * and columns two and three together being used for rows with only one
     * control or multiple equal-sized controls.
     *
     * \param[in] row the number of the first row the backend should start
     * using.
     */
    virtual void main_ui_controls_attach(Gtk::Table &t, unsigned int row);

    /**
     * \brief Returns the number of table rows the backend's secondary tab UI
     * controls will consume.
     *
     * \return the number of rows.
     */
    ...
    /**
     * \brief Returns or allows setting the end of the field the friendly team
     * is defending.
     *
    ...
    /**
     * \brief Returns a signal that fires at the very end of the AI's work each
     * tick.
     *
     ...
     */
    sigc::signal<void, AI::Timestamp, const SSL_WrapperPacket &>
        &signal_vision() const;
     ...
     */
    sigc::signal<void, AI::Timestamp, const SSL_Referee &> &signal_refbox()
        const;

    /**
     * \brief Returns a signal that fires when the visualizer needs an overlay
     * to be drawn.
     *
     * \return the signal.
     */
    sigc::signal<void, Cairo::RefPtr<Cairo::Context>> &signal_draw_overlay()
        const;
   ...
   private:
    ...
    mutable sigc::signal<void, AI::Timestamp, const SSL_WrapperPacket &>
        signal_vision_;
    mutable sigc::signal<void, AI::Timestamp, const SSL_Referee &>
        signal_refbox_;
    mutable sigc::signal<void, Cairo::RefPtr<Cairo::Context>>
        signal_draw_overlay_;
...
class BackendFactory : public Registerable<BackendFactory>
{
   public:
     ...
     * default choice
     */
    virtual std::unique_ptr<Backend> create_backend(
        const std::vector<bool> &disable_cameras,
        int multicast_interface) const = 0;
...
inline sigc::signal<void, AI::Timediff> &AI::BE::Backend::signal_post_tick()
    const
{
    return signal_post_tick_;
}

inline sigc::signal<void, AI::Timestamp, const SSL_WrapperPacket &>
    &AI::BE::Backend::signal_vision() const
{
    return signal_vision_;
}

inline sigc::signal<void, AI::Timestamp, const SSL_Referee &>
    &AI::BE::Backend::signal_refbox() const
{
    return signal_refbox_;
}

inline sigc::signal<void, Cairo::RefPtr<Cairo::Context>>
    &AI::BE::Backend::signal_draw_overlay() const
{
