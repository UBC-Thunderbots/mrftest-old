...
VisionThread::VisionThread(
    MRFDongle &dongle_, int multicast_interface, const std::string &port,
    const std::vector<bool> &disable_cameras_)
    : dongle(dongle_), disable_cameras(disable_cameras_)
{
    vision_thread = std::thread(
        &VisionThread::vision_loop, this, multicast_interface, port);
}

VisionThread::~VisionThread()
{
    // might need to do something here to make sure the thread dies before the
    // dongle does
}
...
void VisionThread::vision_loop(int multicast_interface, std::string port)
{
    ...
    while (stop_thread == false)
    {
        ...
        ssize_t len = recv(
            sock.fd(), buffer, sizeof(buffer),
            0);  // Blocks until packet received
        ...
        {
            std::cout << "warning- throwing out vision packet- queue has 16 "
                         "existing packets"
                      << std::endl;
...
void VisionThread::sock_init(
    FileDescriptor &sock, int multicast_interface, std::string port)
{
    addrinfo hints;
    ...
    if (setsockopt(
            sock.fd(), IPPROTO_IP, IP_ADD_MEMBERSHIP, &mcreq, sizeof(mcreq)) <
        0)
    {
...
void VisionThread::transmit_bots(SSL_WrapperPacket packet)
{
    ...
    const google::protobuf::RepeatedPtrField<SSL_DetectionRobot> &rep =
        vis_inf.friendlyIsYellow() ? det.robots_yellow() : det.robots_blue();
    ...
    for (std::size_t j = 0; j < static_cast<std::size_t>(rep.size()); ++j)
    {
        ...
        // if defending east side, flip the sign of all points (defending end is
        // always negative)
        bool neg = vis_inf.defendingEast();
        Point pos(
            (neg ? -detbot.x() : detbot.x()) / 1000.0,
            (neg ? -detbot.y() : detbot.y()) / 1000.0);
        Angle ori = (Angle::of_radians(detbot.orientation()) +
