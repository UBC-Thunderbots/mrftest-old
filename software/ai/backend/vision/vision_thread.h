...
/* This class is responsible for the following things:
        ...
        -writing vision packets to a queue to be processed further by the main
   thread
...
class VisionThread final : public NonCopyable
{
   public:
    VisionInfo vis_inf;
    static constexpr std::size_t MAX_PATTERN =
        8;  // camera packet structured for robot nums < 8
    VisionThread(
        MRFDongle &dongle_, int multicast_interface, const std::string &port,
        const std::vector<bool> &disable_cameras);
    ~VisionThread();
    void update_info(
        AI::BE::Backend::FieldEnd friendly_side,
        AI::Common::Colour friendly_colour, Point ball_pos);
