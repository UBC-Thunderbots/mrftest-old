		...
		namespace Primitives {
			class PrimitiveDescriptor {
				public:
					...
					PrimitiveDescriptor(Drive::Primitive prim, double p0, double p1, double p2, double p3, uint8_t extra);

					/**
					 * \brief Constructs a primitive descriptor with the given arguments.
					 */
					PrimitiveDescriptor(Drive::Primitive prim, const std::array<double, 4>& arr, uint8_t extra);
			 ...
			 * to place itself on the robot's execution queue. When created, it is added to the queue,
			 * and when deleted, it is removed. If a primitive is destroyed when it isn't the element
			...
			class Primitive : public NonCopyable {
				public:
					 ...
					 */
					Primitive(AI::Common::Player player, Drive::Primitive prim, double p0, double p1, double p2, double p3, uint8_t extra);
