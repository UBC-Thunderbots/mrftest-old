	...
	namespace RobotController {
		...
		class RobotController2 : public ByRef {
			public:
				...
				/**
				 * Multiplies a robot-relative velocity tuple by the wheel matrix to produce a set of wheel rotation speeds.
				 * A robot controller implementation may call this function or may complete ignore it and compute wheel speeds in a different way.
				 *
				 ...
				 *
				 * \param[out] wheel_speeds the wheel speeds, in quarters of a degree of motor shaft rotation per five milliseconds.
				 */
		...
		/**
		 * A compatibility layer for using old controllers that produce output as linear and angular velocities rather than calculating wheel speeds directly.
		 */
		...
		class RobotControllerFactory : public Registerable<RobotControllerFactory> {
			public:
				 ...
				 * (a reference should be kept; the controller must call only inspection methods on the object as mutation methods are intended to be called from the AI).
				 *
				 * \param[in] yellow the colour of the robot (should generally be ignored; intended to be used only in VERY, VERY special situations).
				 *
				 * \param[in] index the pattern index of the robot (should generally be ignored; intended to be used only in VERY, VERY special situations).
				 *
