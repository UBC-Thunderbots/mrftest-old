...
namespace std {
	...
	template<> struct less<AI::Common::Player> final {
		public:
			 ...
			 */
			bool operator()(const AI::Common::Player &x, const AI::Common::Player &y) const;

		private:
			std::less<BoxPtr<AI::BE::Player>> cmp;
	};

	template<> struct hash<AI::Common::Player> final {
		public:
...
namespace AI {
	namespace Common {
		/**
		 * \brief The common functions available on a player in all layers, not including those in Robot
		 */
		class Player {
			public:
				/**
...
inline bool std::less<AI::Common::Player>::operator()(const AI::Common::Player &x, const AI::Common::Player &y) const {
	return cmp(x.impl, y.impl);
}

inline size_t std::hash<AI::Common::Player>::operator()(const AI::Common::Player &x) const {
