...
namespace AI {
	...
	class Player : public Robot {
		public:
			/**
			 * A state block associated with a Player.
			 * %AI classes that need to store state on a per-player basis should subclass this class,
			 * add the necessary state information as fields in the subclass,
			 * and then use the Player::get_state and Player::set_state functions to retrieve and store the state block.
			 */
			 ...
			 *
			 * \return the corresponding State block (which can be cast to a derived type with RefPtr::cast_dynamic),
			 * or a null pointer if no State is associated with the given class.
			 */
			State::Ptr get_state(const std::type_info &tid) const;

			/**
			 * Stores a State block for a class.
			 * Any previously-stored State block for the same class will be dropped.
			 *
			 * \param[in] tid the type of the class whose State should be stored;
			 * you probably want to pass \c typeid(*this) here.
			 *
			 * \param[in] state the new State to store (which can be a null pointer to remove the State altogether).
			 */
		...
		private:
			 ...
			 *
			 * \param[in] yellow \c true if the new robot is yellow, or \c false if it is blue.
			 *
			 * \param[in] pattern_index the index of the vision pattern associated with the player.
			 *
