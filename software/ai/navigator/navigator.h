	...
	namespace Nav {
		...
		class Navigator : public NonCopyable {
			public:
				...
				/**
				 * \brief Reads the requested destinations from the players using W::Player::destination and W::Player::flags, then orders paths to be followed with W::Player::path.
				 */
				virtual void tick() = 0;

				/**
				 * \brief Returns the GTK widget for this Navigator, which will be integrated into the AI's user interface.
				 *
		...
		/**
		 * \brief A NavigatorFactory is used to construct a particular type of Navigator.
		 *
		...
		class NavigatorFactory : public Registerable<NavigatorFactory> {
			public:
				/**
				 * \brief Constructs a new instance of the Navigator corresponding to this NavigatorFactory.
				 *
