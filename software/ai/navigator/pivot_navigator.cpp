...
DegreeParam offset_angle(
    u8"offset angle (degrees)", u8"AI/Nav/Pivot", 80.0, -1000.0, 1000.0);
DoubleParam offset_distance(
    u8"offset distance", u8"AI/Nav/Pivot", 0.1, -1000.0, 10.0);
DegreeParam orientation_offset(
    u8"orientation offset (degrees)", u8"AI/Nav/Pivot", 30.0, -1000.0, 1000.0);
}
...
void PivotNavigator::tick()
{
    for (Player player : world.friendly_team())
    {
        ...
        Player::Path path;
        if ((diff.len() < 0.3 && diff.len() > 0.1) &&
            player.velocity().len() < 0.3 &&
            (world.ball().position() - currentPosition)
                    .orientation()
                    .angle_diff(player.orientation()) < Angle::of_radians(1.2))
        {
            destinationPosition =
                player.position() +
                offset_distance *
                    Point::of_angle(player.orientation() - offset_angle);
        ...
        else
        {
            ...
            destinationOrientation =
                (world.ball().position() - currentPosition).orientation();
