	...
	class PrimTest {
		public:
			...
			PrimTest() {
				...
				activate.signal_clicked().connect(sigc::mem_fun(this, &PrimTest::call_test_fun));
			}
...
*/
	class ShootTest : public PrimTest {
		public:
			...
			ShootTest(World w): PrimTest(), dest(Point()), power(0), chip(false), orient(Angle::zero()), world(w) {
			...
			void build_widget() {
				...
				//link signals
				point_x_slider.signal_value_changed().connect(sigc::mem_fun(this,&ShootTest::on_point_x_changed));
				point_y_slider.signal_value_changed().connect(sigc::mem_fun(this,&ShootTest::on_point_y_changed));
				to_chip.signal_clicked().connect(sigc::mem_fun(this,&ShootTest::on_chip_changed));
				angle_entry.signal_value_changed().connect(sigc::mem_fun(this,&ShootTest::on_angle_changed));
				power_entry.signal_value_changed().connect(sigc::mem_fun(this,&ShootTest::on_power_changed));
			}
	...
	class MoveTest : public PrimTest {
		public:
			...
			MoveTest(World w): PrimTest(), dest(Point(0,0)), orient(Angle::of_degrees(45)), time_delta(0), world(w) {
				...
				tests["MoveTimeDelta"] = static_cast<testfun_t>(&MoveTest::test_move_tdelta_dest);
				tests["MoveOrientTimeDelta"] = static_cast<testfun_t>(&MoveTest::test_move_ori_tdelta_dest);
			...
			void build_widget() {
				point_x_slider.set_range(-world.field().length()/2,world.field().length()/2);
				point_y_slider.set_range(-world.field().width()/2,world.field().width()/2);
				angle_entry.set_range(-360,360); //change range? 360 degrees sounds excessive
				...
				//link signals
				point_x_slider.signal_value_changed().connect(sigc::mem_fun(this,&MoveTest::on_point_x_changed));
				point_y_slider.signal_value_changed().connect(sigc::mem_fun(this,&MoveTest::on_point_y_changed));
				angle_entry.signal_value_changed().connect(sigc::mem_fun(this,&MoveTest::on_angle_changed));
				time_entry.signal_value_changed().connect(sigc::mem_fun(this,&MoveTest::on_time_changed));
			}
	...
	class DribbleTest : public PrimTest {
		public:
			...
			DribbleTest() : dest(Point()), orient(Angle::of_degrees(45)), small_kick_allowed(true), rpm(0) {
	...
	class PivotTest : public PrimTest {
		public:
			...
			PivotTest() : centre(Point()), swing(Angle::of_degrees(45)), orient(Angle::of_degrees(45)) {
	...
	class BezierTest : public PrimTest {
		public:
			...
			BezierTest(Point p0, Point p1, Point p2, double t) : p0(p0), p1(p1), p2(p2), t(t) { }
	};
}
...
void MPTest::build_gui() {
	...
	combo.signal_changed().connect(sigc::mem_fun(this, &MPTest::on_combo_changed));
	test_combo.signal_changed().connect(sigc::mem_fun(this, &MPTest::on_test_combo_changed));
...
void MPTest::on_test_combo_changed() {
	if (current_test->tests.find(test_combo.get_active_text()) == current_test->tests.end()) {
