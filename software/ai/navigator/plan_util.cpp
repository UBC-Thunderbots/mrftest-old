...
namespace {
	...
	double Plan::get_enemy_trespass(Point cur, Point dst, World world) {
		...
		for (Robot rob : world.enemy_team()) {
			...
			double sdist = dist(Seg(rob.position(), rob.position() + ENEMY_MOVEMENT_FACTOR * rob.velocity()), Seg(cur, dst));
			violate = std::max(violate, circle_radius - sdist);
		}
	...
	double Plan::get_friendly_trespass(Point cur, Point dst, World world, Player player) {
		...
		for (Player rob : world.friendly_team()) {
			...
			double sdist = dist(Seg(rob.position(), rob.position() + FRIENDLY_MOVEMENT_FACTOR * rob.velocity()), Seg(cur, dst));
			violate = std::max(violate, circle_radius - sdist);
		}
		return violate;
	}
...
namespace {
	struct Violation final {
		...
		void set_violation_amount(Point cur, Point dst, World world, Player player) {
		...
		explicit Violation(Point cur, Point dst, World world, Player player, MoveFlags added_flags) : enemy(0.0), friendly(0.0), play_area(0.0), ball_stop(0.0), ball_tiny(0.0), ball_regular(0.0), friendly_defense(0.0), enemy_defense(0.0), own_half(0.0), penalty_kick_friendly(0.0), penalty_kick_enemy(0.0), goal_post(0.0), total_bounds(0.0), net_allowance(0.0), extra_flags(added_flags) {
			if (OWN_HALF_OVERRIDE) {
				extra_flags = extra_flags | MoveFlags::STAY_OWN_HALF;
			}
			set_violation_amount(cur, dst, world, player);
		...
		static Violation get_violation_amount(Point cur, Point dst, World world, Player player) {
			Violation v(cur, dst, world, player);
			return v;
		}

		static Violation get_violation_amount(Point cur, Point dst, World world, Player player, MoveFlags extra_flags) {
	...
	void process_obstacle(std::vector<Point> &ans, World world, Player player, Point segA, Point segB, double dist, int num_points) {
		...
		for (Point i : temp) {
			if (Plan::valid_dst(i, world, player)) {
				ans.push_back(i);
			}
		}
	}
...
bool Plan::valid_dst(Point dst, World world, Player player) {
	return Violation::get_violation_amount(dst, dst, world, player).violation_free();
}

bool Plan::valid_path(Point cur, Point dst, World world, Player player) {
...
std::vector<Point> Plan::get_obstacle_boundaries(World world, Player player) {
	return get_obstacle_boundaries(world, player, MoveFlags::NONE);
}

std::vector<Point> Plan::get_obstacle_boundaries(World world, Player player, MoveFlags added_flags) {
...
std::pair<Point, AI::Timestamp> Plan::get_ramball_location(Point dst, World world, Player player) {
...
AI::Timestamp Plan::get_next_ts(AI::Timestamp now, Point &p1, Point &p2, Point target_velocity) {
...
double Plan::estimate_action_duration(std::vector<std::pair<Point, Angle>> path_points) {
	double total_time = 0;
...
/*
bool intercept_flag_handler(World world, Player player,  AI::Nav::RRT::PlayerData::Ptr player_data) {
...
#warning flags and timespec are not accounted for properly
	for (int i = 0; i <= points_to_check; i++) {
		...
		if (estimate_action_duration(path_points_with_angle) < (interval_time * i) || (i == points_to_check) || ball.velocity().len() < CATCH_BALL_VELOCITY_THRESH) {
