...
Defensive3::Defensive3(const RefPtr<World> world) : world(world) {
...
void Defensive3::tick() {
	...
	std::map<RefPtr<Player>, RefPtr<Tactic> > tactics;
	...
	RefPtr<Player> baller;
	std::vector<RefPtr<Player> > defenders;
	for (std::set<RefPtr<Player> >::iterator it = players.begin(); it != players.end(); ++it) {
#warning more than 1 player may posses ball
	...
	std::sort(defenders.begin(), defenders.end(), AIUtil::CmpDist<RefPtr<Player> >(world->ball()->position()));
...
#warning goalie should defend the middle if no other defenders exist
	if (goalierush){ // check if goalie should rush
		...
		RefPtr<Move> tactic(new Move(goalie, world));
		...
		RefPtr<Shoot> shoot_tactic = RefPtr<Shoot>(new Shoot(goalie, world));
		shoot_tactic->force();
	...
	for (size_t i = 0; i < defenders.size(); ++i) {
		if (chaser == defenders[i]) { // should be exact
			RefPtr<Shoot> shoot_tactic = RefPtr<Shoot>(new Shoot(defenders[i], world));
			shoot_tactic->force();
			shoot_tactic->set_pivot(false);
			...
			RefPtr<Move> tactic(new Move(defenders[i], world));
...
void Defensive3::add_player(RefPtr<Player> player) {
	if (!player) {
...
void Defensive3::remove_player(RefPtr<Player> player) {
	if (!player) {
		LOG_ERROR("null player");
		return;
	}
...
void Defensive3::set_goalie(RefPtr<Player> player) {
	if (!player) {
		LOG_ERROR("null player");
		return;
	}
...
RefPtr<Player> Defensive3::pop_player() {
...
void Defensive3::set_chaser(RefPtr<Player> player) {
#warning fix
	if (!player) {
