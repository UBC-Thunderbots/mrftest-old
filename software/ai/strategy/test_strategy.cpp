...
namespace {
	class TestStrategy : public Strategy {
		public:
			TestStrategy(RefPtr<World> world);
			void tick();
	...
	TestStrategy::TestStrategy(RefPtr<World> world) : the_world(world) {
...
//		std::cout << receiver->position() << std::endl;
		...
		RefPtr<Player> passer = the_team.get_player(2);
//		std::cout << passer->position() << std::endl;
		...
		RefPtr<Player> interceptor = the_team.get_player(3);
		...
		{
			RefPtr<Player> the_player = the_team.get_player(i);
...
#warning has_ball
	...
	RefPtr<Strategy2> TestStrategyFactory::create_strategy(RefPtr<World> world) {
		RefPtr<Strategy2> s(new TestStrategy(world));
		return s;
	}
