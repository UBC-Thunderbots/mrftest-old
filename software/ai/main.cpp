...
int app_main(int argc, char **argv)
{
    ...
    Glib::OptionGroup option_group(
        u8"thunderbots", u8"Control Process Options",
        u8"Show Control Process Options");
    ...
    blue_entry.set_description(
        u8"Controls blue robots (default is same as last time)");
    bool blue = false;
    ...
    yellow_entry.set_description(
        u8"Controls yellow robots (default is same as last time)");
    bool yellow = false;
    ...
    east_entry.set_description(
        u8"Defends east goal (default is same as last time)");
    bool east = false;
    ...
    west_entry.set_description(
        u8"Defends west goal (default is same as last time)");
    bool west = false;
    ...
    multicast_interface_entry.set_description(
        u8"Overrides the kernel's default choice of network interface on which "
        u8"to receive multicast packets");
    ...
    disable_camera_entry.set_description(
        u8"Disables reception of packets from a particular camera");
    ...
    high_level_entry.set_description(
        u8"Selects which high level should be selected at startup");
    ...
    navigator_entry.set_description(
        u8"Selects which navigator should be selected at startup");
    ...
    minimize_entry.set_description(
        u8"Starts with the control window minimized");
    bool minimize = false;
    ...
    {
        multicast_interface_index = static_cast<int>(if_nametoindex(
            Glib::locale_from_utf8(multicast_interface_name).c_str()));
        if (!multicast_interface_index)
        {
            throw std::runtime_error(
                Glib::locale_from_utf8(Glib::ustring::compose(
                                           u8"Interface “%1” not found.",
    ...
    if (be == bem.end())
    {
        throw std::runtime_error(Glib::ustring::compose(
            u8"There is no backend “%1”.", backend_name));
    }
    const std::unique_ptr<AI::BE::Backend> &beptr =
        be->second->create_backend(disable_cameras, multicast_interface_index);
    ...
    if (!setup.high_level_name.empty())
    {
        ...
        const Map::const_iterator &i =
            m.find(setup.high_level_name.collate_key());
        if (i != m.end())
        {
            ai.high_level =
                i->second->create_high_level(AI::HL::W::World(backend));
        }
    }
    if (!setup.navigator_name.empty())
    {
        ...
        const Map::const_iterator &i =
            m.find(setup.navigator_name.collate_key());
        if (i != m.end())
        {
            ai.navigator =
                i->second->create_navigator(AI::Nav::W::World(backend));
        }
    }
