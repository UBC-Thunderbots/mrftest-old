...
constexpr uint32_t UPGRADE_AREA_MAGICS[UPGRADE_AREA_COUNT] = {
    UINT32_C(0x1453CABE), UINT32_C(0x74E4BCC5),
};
...
ScanResult::ScanResult(const SectorArray &sarray)
{
    ...
    for (unsigned int i = 0; i != UPGRADE_AREA_COUNT; ++i)
    {
        ...
        if (magic == UPGRADE_AREA_MAGICS[i])
        {
            ...
            if (!(flags & UINT32_C(0xFFFFFFFE)) && length &&
                (length <= (UPGRADE_AREA_SECTORS - 1) * SECTOR_SIZE))
            {
                ...
                for (off_t j = 0; j != length_sectors; ++j)
                {
                    uint32_t sector_base =
                        static_cast<uint32_t>(j * SECTOR_SIZE);
                    std::array<uint8_t, SECTOR_SIZE> sector_data;
                    sarray.get(first_sector + j, &sector_data[0]);
                    actual_crc = CRC32::calculate(
                        &sector_data[0], MIN(SECTOR_SIZE, length - sector_base),
                        actual_crc);
                }
    ...
    off_t low  = UPGRADE_AREA_COUNT * UPGRADE_AREA_SECTORS,
          high = sarray.size() + 1;
    while (low + 1 < high)
    {
        ...
        bool blank =
            std::count(sector.begin(), sector.end(), static_cast<uint8_t>(0)) ==
            static_cast<std::vector<uint8_t>::difference_type>(sector.size());
        if (blank)
        {
            high = pos + 1;
        }
...
const std::array<ScanResult::UpgradeArea, UPGRADE_AREA_COUNT>
    &ScanResult::upgrade_areas() const
{
...
int do_copy(SectorArray &sdcard, const ScanResult *scan_result, char **args)
{
    std::size_t epoch_index =
        static_cast<std::size_t>(std::stoll(args[0], nullptr, 0));
    if (!epoch_index)
    {
    ...
    {
        std::cerr << "This card has only " << scan_result->epochs().size()
                  << " epochs.\n";
        return 1;
    }
    const ScanResult::Epoch &epoch = scan_result->epochs()[epoch_index - 1];

    std::cout << "Copying sectors " << epoch.first_sector << " through "
              << epoch.last_sector << " to file \"" << args[1] << "\": ";
    std::cout.flush();

    FileDescriptor fd(FileDescriptor::create_open(
        args[1], O_WRONLY | O_CREAT | O_TRUNC, 0666));
    for (off_t sector = epoch.first_sector; sector <= epoch.last_sector;
         ++sector)
    {
...
int do_erase(SectorArray &sdcard, const ScanResult *, char **params)
{
    ...
    else
    {
        ...
        if (!found)
        {
            ...
            std::copy(
                UPGRADE_AREA_NAMES.begin(), UPGRADE_AREA_NAMES.end(),
    ...
    blkdiscard_params[1] =
        static_cast<uint64_t>((last - first + 1) * SECTOR_SIZE);
    if (ioctl(sdcard.fd.fd(), BLKDISCARD, blkdiscard_params) < 0)
    {
        if (errno == EOPNOTSUPP)
        {
            std::cout << "Well your card reader dun sucks so I card scans and "
                         "manual overwrite"
                      << std::endl;
            ScanResult scan_result(sdcard);
            constexpr std::size_t WRITE_AMOUNT =
                UPGRADE_AREA_SECTORS * SECTOR_SIZE;
            uint8_t blank_data[WRITE_AMOUNT];
            std::memset(blank_data, 0, WRITE_AMOUNT);
            std::size_t write_size;
            off_t file_offset = first * SECTOR_SIZE;
            off_t past_end_offset =
                MIN(scan_result.nonblank_size(), last + 1) * SECTOR_SIZE;
            do
            {
                write_size = std::min(
                    WRITE_AMOUNT,
                    static_cast<std::size_t>(past_end_offset - file_offset));
                ssize_t ret_val =
                    pwrite(sdcard.fd.fd(), blank_data, write_size, file_offset);
                if (ret_val < 0)
                {
                    throw SystemError("pwrite", errno);
                }
...
int do_info(SectorArray &sdcard, const ScanResult *scan_result, char **)
{
    ...
    for (unsigned int i = 0; i != UPGRADE_AREA_COUNT; ++i)
    {
        std::cout << (i + 1) << " (" << UPGRADE_AREA_NAMES[i]
                  << std::string(
                         max_name_length - UPGRADE_AREA_NAMES[i].size(), ' ')
                  << "): ";
        ...
        switch (area.status)
        {
            ...
            case ScanResult::UpgradeAreaStatus::OK:
                std::cout << "OK (length " << area.length << " bytes, CRC 0x"
                          << tohex(area.crc, 8);
                if (area.ephemeral)
                {
                    std::cout << ", ephemeral";
...
#pragma GCC diagnostic pop
        ...
        std::strftime(
            stamp_formatted, sizeof(stamp_formatted), "%c", std::localtime(&t));
        std::cout << (i + 1) << '@' << stamp_formatted << " + 0."
                  << todecu(epoch.stamp % 1000000, 6) << " s: " << '['
                  << epoch.first_sector << ',' << epoch.last_sector
                  << "] length " << (epoch.last_sector - epoch.first_sector + 1)
                  << '\n';
    }
    return 1;
}
...
int do_install(SectorArray &sdcard, const ScanResult *, char **params)
{
    ...
    unsigned int area = static_cast<unsigned int>(
        std::find(
            UPGRADE_AREA_NAMES.begin(), UPGRADE_AREA_NAMES.end(), area_name) -
        UPGRADE_AREA_NAMES.begin());
    ...
    {
        std::cerr
            << "Third parameter must be flags, one of:\nnormal\nephemeral\n";
        return 1;
    }
    ...
    {
        std::cerr << "File is too large (" << data.size()
                  << " bytes, maximum is "
                  << ((UPGRADE_AREA_SECTORS - 1) * SECTOR_SIZE) << ")\n";
        return 1;
    }
    ...
    if (data.size() % SECTOR_SIZE)
    {
        data.append(
            static_cast<size_t>(SECTOR_SIZE) -
                (data.size() % static_cast<size_t>(SECTOR_SIZE)),
            0);
    }
    ...
    {
        sdcard.put(
            area * UPGRADE_AREA_SECTORS + 1 + i,
            &data[i * static_cast<size_t>(SECTOR_SIZE)]);
    }
    ...
    std::cout << "Image written (" << original_size << " bytes, CRC 0x"
              << tohex(crc, 8) << ").\n";
...
int do_tsv(SectorArray &sdcard, const ScanResult *scan_result, char **args)
{
    ...
    std::cout << "Outputting sectors " << epoch.first_sector << " through "
              << epoch.last_sector << " to TSV file \"" << args[1] << "\": ";
    std::cout.flush();
    ...
    ofs << "DR X (m)\tDR Y (m)\tDR θ (r)\tDR VX (m/s)\tDR VY (m/s)\tDR Vθ "
           "(r/s)\t";
    ...
    ofs << "Dribbler Ticked?\tDribbler PWM (/255)\tDribbler Speed "
           "(rpm)\tDribbler Temp (°C)\tIdle Cycles";
         ...
         ++sector)
    {
        ...
        for (std::size_t record = 0;
             record < static_cast<size_t>(RECORDS_PER_SECTOR); ++record)
        {
            const uint8_t *ptr =
                &buffer[record * static_cast<size_t>(LOG_RECORD_SIZE)];

            // Decode the record.
            uint32_t magic = decode_u32_le(ptr);
            ptr += 4;
            uint32_t record_epoch = decode_u32_le(ptr);
            ptr += 4;
            uint64_t stamp = decode_u64_le(ptr);
            ptr += 8;
            if (magic == LOG_MAGIC_TICK && record_epoch == epoch_index)
            {
                float breakbeam_diff = decode_float_le(ptr);
                ptr += 4;
                float battery_voltage = decode_float_le(ptr);
                ptr += 4;
                float capacitor_voltage = decode_float_le(ptr);
                ptr += 4;

                float dr_data[6];
                for (float &value : dr_data)
                {
                    value = decode_float_le(ptr);
                    ptr += 4;
                }
                ...
                float gyro_avel = decode_float_le(ptr);
                ptr += 4;
                ...
                uint16_t cam_latency = decode_u16_le(ptr);
                ptr += 2;

                unsigned int new_cam_data = decode_u8_le(ptr);
                ptr += 1;
                unsigned int drive_serial = decode_u8_le(ptr);
                ptr += 1;
                unsigned int primitive = decode_u8_le(ptr);
                ptr += 1;
                ...
                {
                    value = static_cast<int16_t>(decode_u16_le(ptr));
                    ptr += 2;
                }
                ...
                {
                    value = decode_u8_le(ptr);
                    ptr += 1;
                }

                uint8_t dribbler_ticked = decode_u8_le(ptr);
                ptr += 1;
                uint8_t dribbler_pwm = decode_u8_le(ptr);
                ptr += 1;
                uint8_t dribbler_speed = decode_u8_le(ptr);
                ptr += 1;
                uint8_t dribbler_temperature = decode_u8_le(ptr);
                ptr += 1;

                uint32_t idle_cycles = decode_u32_le(ptr);
                ptr += 4;
                ...
                ofs << epoch_index << '\t' << (stamp / 1000000) << '.'
                    << todecu(stamp % 1000000, 6) << '\t' << breakbeam_diff
                    << '\t' << battery_voltage << '\t' << capacitor_voltage;
                ...
                ofs << '\t' << static_cast<unsigned int>(dribbler_pwm) << '\t'
                    << static_cast<unsigned int>(dribbler_speed);
...
struct Command final
{
    ...
    bool needs_scan;
    int (*handler)(
        SectorArray &sdcard, const ScanResult *scan_result, char **args);
};
...
int app_main(int argc, char **argv)
{
    ...
    {
        std::cerr << "Wrong number of parameters for command "
                  << command->command << ", found " << (argc - 3)
                  << ", expected " << command->args << ".\n";
        return 1;
    }

    // Open the device.
    FileDescriptor sdfd(FileDescriptor::create_open(
        argv[1], command->needs_write ? O_RDWR : O_RDONLY, 0));
