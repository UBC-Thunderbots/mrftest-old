...
namespace
{
class MessagesALM final : public AbstractListModel,
                          public Glib::Object,
                          public NonCopyable
{
   public:
   ...
   private:
    explicit MessagesALM() : Glib::ObjectBase(typeid(MessagesALM))
    {
        ...
        Annunciator::signal_message_activated.connect(
            sigc::mem_fun(this, &MessagesALM::on_message_activated));
        Annunciator::signal_message_deactivated.connect(
            sigc::mem_fun(this, &MessagesALM::on_message_deactivated));
        Annunciator::signal_message_text_changed.connect(
            sigc::mem_fun(this, &MessagesALM::on_message_text_changed));
        Annunciator::signal_message_aging.connect(
            sigc::mem_fun(this, &MessagesALM::on_message_aging));
        Annunciator::signal_message_reactivated.connect(
            sigc::mem_fun(this, &MessagesALM::on_message_reactivated));
        Annunciator::signal_message_hidden.connect(
            sigc::mem_fun(this, &MessagesALM::on_message_hidden));
    }
...
void message_cell_data_func(
    Gtk::CellRenderer *r, const Gtk::TreeModel::iterator &iter)
{
    Gtk::CellRendererText *rt = dynamic_cast<Gtk::CellRendererText *>(r);
    rt->property_text() =
        iter->get_value(MessagesALM::instance()->message_column);
    bool active = iter->get_value(MessagesALM::instance()->active_column);
    Annunciator::Message::Severity severity =
        iter->get_value(MessagesALM::instance()->severity_column);
    rt->property_foreground() =
        active && severity == Annunciator::Message::Severity::HIGH ? u8"white"
                                                                   : u8"black";
    rt->property_background() =
        active ? (severity == Annunciator::Message::Severity::HIGH ? u8"red"
                                                                   : u8"yellow")
               : u8"white";
}
}
...
GUIAnnunciator::GUIAnnunciator() : view(MessagesALM::instance())
{
    ...
    message_column->set_cell_data_func(
        message_renderer, &message_cell_data_func);
    add(view);
