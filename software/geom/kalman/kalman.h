...
class Kalman {
	public:
		 ...
		 *
		 * \param[in] measure_std the standard deviation of measurements fed to update(double, std::chrono::steady_clock::time_point).
		 *
		 ...
		 */
		void predict(Timestamp prediction_time, Matrix &state_predict, Matrix &p_predict) const;
	...
	private:
		...
		void predict_step(Timediff timestep, double control, Matrix &state_predict, Matrix &p_predict) const;
