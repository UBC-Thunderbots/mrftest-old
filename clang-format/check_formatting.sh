...
output="$($CURR_DIR/git-clang-format --binary $CURR_DIR/clang-format-$CLANG_VERSION --commit origin/dev --diff)"
...
if [[ $output == *"no modified files to format"* ]] || [[ $output == *"clang-format did not modify any files"* ]] ; then
    ...
    # If we failed, echo the results of clang-format so we can see what we should changet a
    echo "$output"
    echo "=================================================="
