...
sigc::signal<void, Log::DebugMessageLevel, const Glib::ustring &>
    ...
    const Glib::ustring &msg, Log::DebugMessageLevel level)
{
    ...
    std::use_facet<std::time_put<wchar_t>>(std::locale())
        .put(
            timestring, timestring, L' ', std::localtime(&stamp), TIME_PATTERN,
            TIME_PATTERN + std::wcslen(TIME_PATTERN));
    ...
    const Glib::ustring &composed = Glib::ustring::compose(
        u8"[%1] [%2:%3] [%4] %5", timestring.str(), file, line, function, msg);
    if (level_name)
    {
