 ...
 *
 * \return \c true if \p elem exists in the range [\p begin, \p end), or \c
 * false if not.
 */
 ...
 *
 * \return the nearest value to \p value that lies in the range [\p lower, \p
 * upper].
 */
template <typename T>
...
/**
 * \brief A comparator that orders small nonnegative integers based on the
 * ordering of objects in a vector at corresponding positions.
 *
...
class IndexComparator final
{
   public:
     ...
     *
     * \param[in] tbl the lookup table to use, which is not copied and must
     * remain valid until the IndexComparator has been destroyed.
     *
...
template <typename T>
inline T clamp_symmetric(const T &value, const T &limit)
{
    return clamp(value, std::min(-limit, limit), std::max(-limit, limit));
}

template <typename T, typename Comp>
inline IndexComparator<T, Comp>::IndexComparator(
    const std::vector<T> &tbl, Comp comp)
    : tbl(tbl), comp(comp)
{
}

template <typename T, typename Comp>
inline bool IndexComparator<T, Comp>::operator()(
