...
bool AbstractListModel::iter_next_vfunc(
    const iterator &iter, iterator &next) const
{
...
bool AbstractListModel::iter_children_vfunc(
    const iterator &, iterator &next) const
{
    make_iter_invalid(next);
    return false;
}

bool AbstractListModel::iter_parent_vfunc(
    const iterator &, iterator &next) const
{
    make_iter_invalid(next);
    return false;
...
bool AbstractListModel::iter_nth_child_vfunc(
    const iterator &, int, iterator &next) const
{
    make_iter_invalid(next);
    return false;
}
...
Gtk::TreeModel::Path AbstractListModel::get_path_vfunc(
...
void AbstractListModel::get_value_vfunc(
    const iterator &iter, int col, Glib::ValueBase &value) const
{
    assert(is_valid_column(col));
    std::size_t index = of_ptr(iter.gobj()->user_data);
...
void AbstractListModel::set_value_impl(
    const iterator &iter, int col, const Glib::ValueBase &value)
{
    assert(is_valid_column(col));
    std::size_t index = of_ptr(iter.gobj()->user_data);
