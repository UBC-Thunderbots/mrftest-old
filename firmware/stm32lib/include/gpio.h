 ...
 * \return the mode
 */
#define gpio_get_mode_raw(port, bit) ((GPIO_MODE_t) (((port).MODER >> (2 * (bit))) & GPIO_MODE_MASK))
 ...
 * \return the speed
 */
#define gpio_get_speed_raw(port, bit) ((GPIO_OSPEED_t) (((port).OSPEEDR >> (2 * (bit))) & GPIO_OSPEED_MASK))
 ...
 * \return the pull direction
 */
#define gpio_get_pupd_raw(port, bit) ((GPIO_PUPD_t) (((port).PUPDR >> (2 * (bit))) & GPIO_PUPD_MASK));
 ...
 * \param[in] level \c true to set the pin, or \c false to reset it
 */
#define gpio_set_output_raw(port, bit, level) gpio_set_reset_mask(port, (level) ? (1 << (bit)) : 0, 1 << (bit))
