...
static float downtime_calc(float vcap, float vbat) {
	...
	if (vcap - vbat >= 1.0f) {
		...
		// vcap ~= vbat so the denominator approaches zero and the equation above falls apart.
		// Use a very conservative value instead.
		time = 0.002f;
	}
...
void charger_mark_fired(void) {
	__atomic_store_n(&full, false, __ATOMIC_RELAXED); // Because energy has been used, capacitors are no longer full.
...
void charger_tick(bool charger_enabled) {
	...
	if (vcap < (vbat - 0.7f) * 0.75f) {
		...
		// Even with no pulses going out, the battery should drain at DC through the inductor and diode into the capacitor.
	...
	if (charger_enabled) {
		...
		arrTicks = auto_reload_register( vcap, vbat ); //total amount of ticks for one cycle
		TIM10_14_CR1_t cr1 = TIM10.CR1;
		cr1.UDIS = 1;
		TIM10.CR1 = cr1; // Disable update events while writing to ARR and CCR1 to ensure atomicity.
		...
		__atomic_fetch_sub(&timeout_counter, 1U, __ATOMIC_RELAXED); // Tick the timeout counter.
	} else {	
		TIM10.CCR1 = 0; //sets duty cycle to 0
		__atomic_store_n(&timeout_counter, CHARGE_TIMEOUT, __ATOMIC_RELAXED); // Reset timeout counter.
	}
}
