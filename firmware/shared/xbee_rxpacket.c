 ...
 */
static xbee_rxpacket_t xbee_packet1, xbee_packet2, xbee_packet3, xbee_packet4, xbee_packet5, xbee_packet6;
...
void xbee_rxpacket_init(void) {
	if (!inited) {
		...
		STACK_PUSH(pending_stack, &xbee_packet1);
		STACK_PUSH(pending_stack, &xbee_packet2);
		STACK_PUSH(pending_stack, &xbee_packet3);
		STACK_PUSH(pending_stack, &xbee_packet4);
		STACK_PUSH(pending_stack, &xbee_packet5);
		STACK_PUSH(pending_stack, &xbee_packet6);
...
\
	...
	if (RCSTA ## usartidx ## bits.OERR) { \
		...
		/* Record the error. */ \
		error_reporting_add(FAULT_XBEE0_OERR_HW + xbeeidx); \
...
\
	...
	if (RCSTA ## usartidx ## bits.FERR) { \
		...
		/* Record the error. */ \
		error_reporting_add(FAULT_XBEE0_FERR + xbeeidx); \
...
\
		case STATE_EXPECT_LENGTH_MSB: \
			...
			if (ch == 0) { \
				...
				/* Record the error. */ \
				error_reporting_add(FAULT_XBEE0_NONZERO_LENGTH_MSB + xbeeidx); \
...
\
		case STATE_EXPECT_LENGTH_LSB: \
			if (ch < 1 || ch > 111) { \
				 ...
				 * Report the error. */ \
				error_reporting_add(FAULT_XBEE0_INVALID_LENGTH_LSB + xbeeidx); \
				/* There's no way to know how long the packet should be, so just go back to looking for SOP. */ \
				rxstates[xbeeidx].state = STATE_EXPECT_SOP; \
				 ...
				 * Report the error. */ \
				error_reporting_add(FAULT_XBEE0_OERR_SW + xbeeidx); \
...
\
		case STATE_EXPECT_CHECKSUM: \
			...
			if (rxstates[xbeeidx].checksum == 0xFF) { \
				 ...
				 * Record the error. */ \
				error_reporting_add(FAULT_XBEE0_CHECKSUM_FAILED + xbeeidx); \
			} \
