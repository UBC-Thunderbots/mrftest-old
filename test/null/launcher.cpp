...
{
    set_title(u8"Tester");
    robot_toggle.signal_toggled().connect(
        sigc::mem_fun(this, &NullTesterLauncher::on_robot_toggled));
    vbox.pack_start(robot_toggle, Gtk::PACK_SHRINK);
    mapper_toggle.signal_toggled().connect(
        sigc::mem_fun(this, &NullTesterLauncher::on_mapper_toggled));
...
void NullTesterLauncher::on_robot_toggled()
{
    if (robot_toggle.get_active())
    {
        ...
        window->signal_delete_event().connect(sigc::bind_return(
            sigc::hide(sigc::bind(
                sigc::mem_fun(&robot_toggle, &Gtk::ToggleButton::set_active),
                false)),
            false));
    }
    else
