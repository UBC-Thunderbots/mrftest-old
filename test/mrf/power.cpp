...
{
    power_drivetrain_button.signal_clicked().connect(
        sigc::mem_fun(this, &PowerPanel::power_drivetrain));
    reboot_button.signal_clicked().connect(
        sigc::mem_fun(this, &PowerPanel::reboot));
    shut_down_button.signal_clicked().connect(
        sigc::mem_fun(this, &PowerPanel::shut_down));
...
void PowerPanel::send_message(uint8_t code, Gtk::Button &button)
{
    ...
    message.reset(new MRFDongle::SendReliableMessageOperation(
        dongle, index, 3, &code, sizeof(code)));
    message->signal_done.connect(
        sigc::mem_fun(this, &PowerPanel::check_result));
}
...
void PowerPanel::check_result(AsyncOperation<void> &op)
{
    ...
    Glib::signal_timeout().connect_once(
        sigc::mem_fun(this, &PowerPanel::timeout), 2000);
}
